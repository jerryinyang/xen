...
Xen uses a **1-minute-bar base data + on-demand generation** architecture. The cAlgo robot collects and stores completed 1-minute bars only. All chart-type transformations are generated by deterministic Python scripts.
...
### Generated Data (Python On-Demand)

Chart-type data is generated by Python generators from 1-minute time bars. It is not stored persistently by default. Frequently reused canonical variants may be persisted under `data/<chart_type>/` only when the generator version and parameters are recorded and the variant can be invalidated if generator logic changes.
...
Each generator produces chart-type DataFrames with a `SourceCloseTime` column (or equivalent) linking chart-type events to real-time coordinates for return evaluation.
...
| **No strategy optimisation in Phase 1** | Phase 1 is characterisation only. No strategy returns, no parameter tuning, no predictive modelling. |
...
| Module | Path | Purpose |
|--------|------|---------|
| Chart-type generators | `python/src/linebreak_generator.py` | Line Break bar generation |
| Chart-type generators | `python/src/renko_generator.py` | Renko brick generation |
| Chart-type generators | `python/src/heiken_ashi_generator.py` | Heiken Ashi candle generation |
...
| Comparative (across chart types/instruments) | 2-4 | 3-5 | 1-2 |
