...
domains=(5m 1h 4h)
strategies=(AvwapBaseline Donchian20)
timeframes=(m1)
...
run_backtest() {
  ...
  local container="ctrader-exp023-$(printf '%s' "$run_id" | tr '[:upper:]' '[:lower:]' | tr '_' '-')-$$"
...
if [[ "${1:-}" == "one" ]]; then
  if [[ $# -ne 4 ]]; then
    echo "Usage: $0 one <SYMBOL> <DOMAIN:5m|1h|4h> <STRATEGY:AvwapBaseline|Donchian20>" >&2
    exit 2
  fi
  symbols=("$2")
  domains=("$3")
