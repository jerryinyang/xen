...
Re-runs the VAL-001 rev. 3 temporal-integrity suite, unchanged, over the
new-universe base files collected by INFR-002 (Phase 010 design §5/C1). The
script checks timestamp alignment, resampling correctness against an
independent oracle, and row-level no-look-ahead via prefix stability over the
analysis slice only. It does not compute returns, signals, P&L, or strategy
metrics.

Scope difference vs VAL-001: file discovery is restricted to base files for
instruments OUTSIDE the already-validated existing universe (BTCUSD, EURUSD,
USTEC, XAUUSD) and excludes pre-sliced analysis exports (``analysis70`` etc.).
All check logic, negative controls, and probe bounds are identical to VAL-001.
A PASS admits the new-universe data for future phases; the final 30% of each
file remains sealed global holdout (only the first 70% is ever collected).
...
# Instruments already validated by VAL-001 (rev. 3) — excluded here so this run
# covers exactly the INFR-002 new-universe files, whatever broker-specific
# symbol names they land under.
EXISTING_UNIVERSE = frozenset({"BTCUSD", "EURUSD", "USTEC", "XAUUSD"})
...
def list_timebar_files() -> list[Path]:
    """Base files for new-universe instruments only.

    Excludes pre-sliced analysis exports and the four existing-universe
    instruments already validated by VAL-001.
    """
    files = []
    for path in sorted((DATA_DIR / "timebars").glob("timebars_*.parquet")):
        if any(marker in path.stem for marker in ANALYSIS_SLICE_MARKERS):
            continue
        if infer_instrument(path) in EXISTING_UNIVERSE:
            continue
        files.append(path)
    return files
...
def plot_validation_status(checks_df: pl.DataFrame, output_path: Path) -> None:
    ...
    if rows:
        ...
        ax.set_title("VAL-003 Validation Status by View (incl. negative controls)")
...
def plot_event_density(densities_df: pl.DataFrame, output_path: Path) -> None:
    ...
    if rows:
        ...
        ax.set_title("VAL-003 Chart Event Density")
        ax.invert_yaxis()
    else:
...
def run_validation() -> tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame, dict[str, Any]]:
    ...
    metadata: dict[str, Any] = {
        ...
        "excluded_existing_universe": sorted(EXISTING_UNIVERSE),
        "excluded_analysis_slice_markers": list(ANALYSIS_SLICE_MARKERS),
        "suite_lineage": "VAL-001 rev. 3 checks, unchanged",
    }
    ...
    if not files:
        add_check(
            ...
            detail=(
                "No new-universe base files found under data/timebars/ "
                "(existing universe and analysis-slice exports are excluded); "
                "run tools/ctrader-cli/run-infr002-collection.sh first"
            ),
        )
        return (
...
def main() -> None:
    configure_logging()
    ensure_output_dirs()
    log_section("VAL-003 New-Universe Data Integrity Validation")
