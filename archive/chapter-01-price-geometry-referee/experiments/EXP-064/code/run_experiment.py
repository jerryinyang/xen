"""EXP-064 — MA(20,50)-Substrate Favourable-Target Geometry (dual-object).

``CF-HA-HARAMI-001`` / HYP-017 — Phase 015 **surface S1** (**dual conditioning object**).
...
**Re-run under D0 Amendment 001 (2026-06-17).** The prior EXP-064 scope measured a single MA
favourable-target axis labelled *hybrid* but reconciled its benchmark arm to EXP-061 ``M0`` -- the
**native** object. This dual-object re-run emits the full 8-variant favourable-target axis **for both
...
EXP-064 was paused (no ``results/``), so resumption is dual-object from the start.

**The question (scope §Hypothesis), per object.** Does varying **only the favourable target** --
from the benchmark ``0.50*M_sofar`` level to a volume-profile level of the prior *completed MA
segment* (``/VPTARGET``: POC / near-VA / far-VA) or a trailing-MA-segment-magnitude distance
(``/MAGTARGET``: ``frac in {0.5,1.0} x W in {5,20}``) -- improve the conditioned harami's gross
per-event **median** expectancy on the MA(20,50) substrate (variant median CI_low>0, >=30 events),
**beat its own object's matched-random-on-MA null** (binding signal attribution, P5), **and beat
that object's benchmark MA variant** (paired median contrast), clearing **P11 with the P6 non-4h
rule**? The adverse target is held at the benchmark **1:1** model and the third barrier at the
**MA-defined adaptive cap** (OAT on the favourable leg only).

The two conditioning objects share the **same** frozen HA-harami detection and the **same** MA
outcome geometry (``rd`` / ``M_sofar`` / favourable references / cap, all from the shared MA
in-progress state); they differ **only** in the ``/STRONG-STAT`` conditioning filter (P2):
  ...
  * **hybrid** (``hyb``) -- ``/STRONG-STAT`` p75 on the in-progress confirmed **ZigZag move** (mask
    ...
    favourable axis; population reconciles to EXP-053's 3202-class set (via the hybrid mask).
...
Favourable-leg primitives reuse ``xen.favourable_targets`` wholesale (substrate-generic; pointed at
the MA-segment references). The benchmark adverse (1:1), the MA-defined adaptive cap, the P15
resolver, the realised returns, the median/mean/trim/tail bootstrap, and the matched-random selector
are reused from the EXP-061/063 dual-object MA pipeline.

Detection on HA candles; every outcome metric on real prices; MA(20,50) on real close. ``TickVolume``
is a broker tick-count proxy for the ``/VPTARGET`` volume profile (disclosed). Outputs under
...
conditioning arm and the full ZigZag-substrate favourable surface (incl. the single ZigZag benchmark
contrast). The binding dual-object MA favourable surface and the P4 mean diagnostic are fully computed.
"""
...
from xen.favourable_targets import (  # noqa: E402
    VP_BIN_FRACTION,
    VP_MIN_REF_BARS,
    VP_VALUE_AREA,
...
# --------------------------------------------------------------------------- #
# Constants (Phase 014-B / 015 D0 frozen + EXP-056/060/061 inherited; no tuning)
# --------------------------------------------------------------------------- #
...
TICKVOL_PROXY = ("TickVolume is broker tick count, a proxy for traded volume; disclosed for every "
                 "/VPTARGET result (the volume profile uses it as the only volume input).")
DE30_DISCLOSURE = (
...
# --------------------------------------------------------------------------- #
# Each object resolves the full 8-variant favourable axis on the SAME MA geometry; only
...
OBJECT_NAME: dict[str, str] = {
    ...
    "hyb": "hybrid (ZigZag /STRONG-STAT x MA geometry; reconciles EXP-061 H0, NEW favourable axis)",
}
OBJECT_BENCH_LABEL: dict[str, str] = {"nat": "M0", "hyb": "H0"}     # EXP-061 reconciliation label

# --------------------------------------------------------------------------- #
# Favourable-target variant set (OAT on the favourable leg; MA substrate)
# --------------------------------------------------------------------------- #
# BENCH (0.50*M_sofar) is the reference + P12 reconciliation anchor. The 7 ALTERNATIVE
# variants (3 /VPTARGET on the prior completed MA segment + 4 /MAGTARGET) are the binding
# lever set. The adverse (1:1) + MA adaptive cap are held at benchmark for every variant.
@dataclass(frozen=True)
class VariantSpec:
    """One predeclared favourable-target variant."""

    vid: str
    idx: int                       # stable RNG / column offset (0..8)
    kind: str                      # "bench" | "vp" | "mag"
    vp_level: str | None = None    # "poc" | "near" | "far"
    vp_inprog: bool = False
    mag_frac: float | None = None
    mag_window: int | None = None


BINDING_SPECS: tuple[VariantSpec, ...] = (
    VariantSpec("BENCH", 0, "bench"),
    VariantSpec("VP-POC", 1, "vp", vp_level="poc"),
    VariantSpec("VP-NEAR", 2, "vp", vp_level="near"),
    VariantSpec("VP-FAR", 3, "vp", vp_level="far"),
    VariantSpec("MAG-0.5x5", 4, "mag", mag_frac=0.5, mag_window=5),
    VariantSpec("MAG-1.0x5", 5, "mag", mag_frac=1.0, mag_window=5),
    VariantSpec("MAG-0.5x20", 6, "mag", mag_frac=0.5, mag_window=20),
    VariantSpec("MAG-1.0x20", 7, "mag", mag_frac=1.0, mag_window=20),
)
DISCLOSED_SPEC = VariantSpec("VP-POC-INPROG", 8, "vp", vp_level="poc", vp_inprog=True)
SPEC_BY_ID: dict[str, VariantSpec] = {v.vid: v for v in (*BINDING_SPECS, DISCLOSED_SPEC)}
VARIANTS: tuple[str, ...] = tuple(v.vid for v in BINDING_SPECS)        # the 8 binding arms
ALT_VARIANTS: tuple[str, ...] = tuple(v.vid for v in BINDING_SPECS if v.vid != "BENCH")
FAV_MODEL: dict[str, str] = {
    "BENCH": "benchmark 0.50*M_sofar (MA-defined; reconciles EXP-061 M0/H0)",
    "VP-POC": "/VPTARGET POC of prior completed MA segment",
    "VP-NEAR": "/VPTARGET near value-area edge (prior completed MA segment)",
    "VP-FAR": "/VPTARGET far value-area edge (prior completed MA segment)",
    "MAG-0.5x5": "/MAGTARGET 0.5 x median(trailing-5 MA-segment magnitudes)",
    "MAG-1.0x5": "/MAGTARGET 1.0 x median(trailing-5 MA-segment magnitudes)",
    "MAG-0.5x20": "/MAGTARGET 0.5 x median(trailing-20 MA-segment magnitudes)",
    "MAG-1.0x20": "/MAGTARGET 1.0 x median(trailing-20 MA-segment magnitudes)",
    "VP-POC-INPROG": "/VPTARGET POC of in-progress MA segment (disclosed secondary; never binding)",
}
...
def ma_segment_moves(ohlc: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    segment's real bars (the /VPTARGET reference span).
    """
...
# --------------------------------------------------------------------------- #
# Pure computation -- favourable-target build (dispatch over the variant set)
# --------------------------------------------------------------------------- #
# The favourable level varies one-at-a-time; the adverse is the benchmark 1:1 model
# (adv = C - rd*fav_dist, returned by the xen.favourable_targets builders) and the third
# barrier is the MA adaptive cap (applied by resolve_arm via bench_n). All references are
# the MA segments (ma_segment_moves): VP = prior completed MA segment bars; MAG = trailing
# MA-segment magnitudes. Real prices + TickVolume only.
def vp_levels_per_event(
    entry_idx: np.ndarray, state: InProgressState, seg: dict[str, np.ndarray],
    ohlc: dict[str, np.ndarray], atr_entry: np.ndarray, in_progress: bool,
) -> dict[str, np.ndarray]:
    """POC / VAL / VAH per entry from the reference MA segment's real bars + TickVolume.

    Reference span: the prior **completed** MA segment ``M_k`` (``[start_idx_k,
    end_idx_k]``; causal -- all bars ``<= end_idx_k = confirm_idx_k <= entry``) or, for
    the disclosed in-progress variant, ``(end_idx_k, entry_idx]``. Spans with
    ``< VP_MIN_REF_BARS`` real bars are insufficient-profile (disclosed). Bounded
    sequential scan per event (one ``volume_profile_levels`` call); no look-ahead.
    """
    ...
    for e in range(n):
        if not state.valid[e]:
            continue
        k = int(state.k[e])
        if k < 0:
            continue
        if in_progress:
            a, b = int(seg["end_idx"][k]) + 1, int(entry_idx[e])
        else:
            a, b = int(seg["start_idx"][k]), int(seg["end_idx"][k])
        if b < a:
            continue
        enough[e] = (b - a + 1) >= VP_MIN_REF_BARS
        if not enough[e]:
            continue
        lv = volume_profile_levels(ohlc["low"][a:b + 1], ohlc["high"][a:b + 1],
                                   ohlc["volume"][a:b + 1], VP_BIN_FRACTION * float(atr_entry[e]))
        if lv.defined:
            defined[e] = True
            poc[e], val[e], vah[e] = lv.poc, lv.val, lv.vah
    return {"poc": poc, "val": val, "vah": vah, "defined": defined, "enough": enough}


def build_favourable(
    spec: VariantSpec, entry_idx: np.ndarray, entry_epoch: np.ndarray,
    entry_close: np.ndarray, state: InProgressState, seg: dict[str, np.ndarray],
    ohlc: dict[str, np.ndarray], atr_entry: np.ndarray,
) -> dict[str, np.ndarray]:
    """Build ``(fav, adv, fav_dist, ok, reason_*)`` for one favourable variant.

    ``ok`` marks events whose variant target is built and lies on the reversal side of
    ``C`` (``fav_dist > 0``). The mutually-exclusive ``reason_validity`` / ``reason_profile``
    / ``reason_warmup`` masks record *why* an otherwise in-progress event is excluded
    (disclosed counts; never silently clamped). The adverse is the benchmark 1:1 model.
    Real prices + TickVolume only.
    """
    rd = state.rd
    if spec.kind == "bench":
        return _bench_targets(entry_close, rd, state.m_sofar)
    if spec.kind == "mag":
        return _mag_targets(entry_epoch, entry_close, rd, seg,
                            int(spec.mag_window), float(spec.mag_frac))
    vp = vp_levels_per_event(entry_idx, state, seg, ohlc, atr_entry, spec.vp_inprog)
    return _vp_targets(entry_close, rd, vp, spec.vp_level)


def _zero_reasons(n: int) -> dict[str, np.ndarray]:
    return {"reason_validity": np.zeros(n, dtype=bool),
            "reason_profile": np.zeros(n, dtype=bool),
...
def _bench_targets(
    ...
    """Benchmark 50%-of-M_sofar favourable target (reproduces EXP-061 M0/H0 exactly)."""
    bar = benchmark_barriers(entry_close, rd, m_sofar)
    ok = bar["fav_dist"] > 0.0
    res = _zero_reasons(int(ok.shape[0]))
    res["reason_validity"] = ~ok
    return {"fav": bar["fav"], "adv": bar["adv"], "fav_dist": bar["fav_dist"], "ok": ok, **res}


def _mag_targets(
    entry_epoch: np.ndarray, entry_close: np.ndarray, rd: np.ndarray,
    seg: dict[str, np.ndarray], window: int, frac: float,
) -> dict[str, np.ndarray]:
    """Trailing-MA-segment-magnitude favourable distance (``/MAGTARGET``)."""
    fav_dist, defined = trailing_magnitude_distance(
        entry_epoch, seg["confirm_epoch"], seg["magnitude"], window, frac)
    bar = barriers_from_distance(entry_close, rd, fav_dist)
    ok = defined & bar["valid"]
    res = _zero_reasons(int(ok.shape[0]))
    res["reason_warmup"] = ~defined
    res["reason_validity"] = defined & ~bar["valid"]
    return {"fav": bar["fav"], "adv": bar["adv"], "fav_dist": bar["fav_dist"], "ok": ok, **res}


def _vp_targets(
    entry_close: np.ndarray, rd: np.ndarray, vp: dict[str, np.ndarray], level: str,
) -> dict[str, np.ndarray]:
    """Volume-profile favourable level (POC / nearer-VA / farther-VA edge)."""
    if level == "poc":
        fav_level = vp["poc"]
    else:
        fav_level = _pick_va_edge(entry_close, rd, vp["val"], vp["vah"], level)
    bar = barriers_from_fav_level(entry_close, rd, fav_level)
    ok = vp["enough"] & vp["defined"] & bar["valid"]
    res = _zero_reasons(int(ok.shape[0]))
    res["reason_profile"] = ~vp["enough"] | (vp["enough"] & ~vp["defined"])
    res["reason_validity"] = vp["enough"] & vp["defined"] & ~bar["valid"]
    return {"fav": bar["fav"], "adv": bar["adv"], "fav_dist": bar["fav_dist"], "ok": ok, **res}


def _pick_va_edge(
    entry_close: np.ndarray, rd: np.ndarray, val: np.ndarray, vah: np.ndarray, level: str,
) -> np.ndarray:
    """Choose the nearer/farther valid value-area edge (by reversal distance)."""
    val_d, vah_d = rd * (val - entry_close), rd * (vah - entry_close)
    val_ok = np.isfinite(val) & (val_d > 0.0)
    vah_ok = np.isfinite(vah) & (vah_d > 0.0)
    both = val_ok & vah_ok
    if level == "near":
        pick_val = np.where(both, val_d <= vah_d, val_ok)
    else:                                                  # far
        pick_val = np.where(both, val_d >= vah_d, val_ok)
    chosen = np.where(pick_val, val, vah)
    return np.where(val_ok | vah_ok, chosen, np.nan)
...
def resolve_arm(
    ...
    """Resolve one favourable level + the benchmark 1:1 adverse over ``population``.

    Reuses :func:`resolve_path_ordered` (the exact EXP-053 path: the supplied favourable
    level, the benchmark 1:1 adverse, the MA adaptive cap ``n_event``). The median path
    (``rng``) reproduces EXP-061 M0/H0 for BENCH; the mean CI (``mean_rng``) and the 10%
    trimmed-mean CI (``trim_rng``) use dedicated streams.
    """
...
def signal_arm(
    ...
    cond_mask: np.ndarray, entry_idx: np.ndarray, entry_epoch: np.ndarray, cell_index: int,
) -> tuple[ArmResult, np.ndarray, dict[str, np.ndarray]]:
    ...
    (rd / M_sofar / cap / VP & MAG references) is shared. Returns the ``ArmResult``, the
    per-event ``fav_dist`` (for the validity invariant), and the per-event exclusion-reason
    masks. The (object, BENCH) arm reuses the precomputed benchmark fav/adv + its object's
    M0/H0 RNG purposes so it reproduces EXP-061 M0 (native) / H0 (hybrid) exactly.
    """
    ...
    if variant == "BENCH":
        ...
        ok = ma["fav_dist"] > 0.0
        reasons = _zero_reasons(int(fav_dist.shape[0]))
        reasons["reason_validity"] = ~ok
    else:
        bld = build_favourable(SPEC_BY_ID[variant], entry_idx, entry_epoch, ma["entry_close"],
                               state, ma["seg"], ohlc, ma["atr_entry"])
        fav, adv, fav_dist, ok = bld["fav"], bld["adv"], bld["fav_dist"], bld["ok"]
        reasons = {k: bld[k] for k in ("reason_validity", "reason_profile", "reason_warmup")}
...
def matched_random_arm(
    ...
    if variant == "BENCH":
        ...
        bld = build_favourable(SPEC_BY_ID[variant], drawn, ohlc["epoch"][drawn],
                               ohlc["close"][drawn], sub, seg, ohlc, atr_all[drawn])
...
def paired_vs_bench(variant: ArmResult, bench: ArmResult, cell_index: int,
    ...
    binding EXP-056 lever (does the variant beat the object's benchmark median?).
    """
...
def compute_cell(
    ...
    arms, fav_dists, disclosed = _resolve_objects(
        ohlc, ma, cond_masks, entry_idx, entry_epoch, cell_index)
    n_conditioned = {o: int((ma["buildable"] & cond_masks[o]).sum()) for o in OBJECTS}
    return {
        **base, "empty": False, "arms": arms, "disclosed": disclosed,
        "n_conditioned": n_conditioned,
        "causality_ok": _causality_ok(ohlc, entry_idx, entry_epoch, ma, zz),
        "invariants": {o: _cell_invariants(arms[o], fav_dists[o]) for o in OBJECTS},
    }


def _resolve_objects(
    ohlc: dict[str, np.ndarray], ma: dict[str, Any], cond_masks: dict[str, np.ndarray],
    entry_idx: np.ndarray, entry_epoch: np.ndarray, cell_index: int,
) -> tuple[dict[str, Any], dict[str, dict[str, np.ndarray]], dict[str, ArmResult]]:
    """Resolve the 8 favourable variants (signal + matched-random) + contrasts, per object."""
    seg = ma["seg"]
    ...
    arms: dict[str, Any] = {}
    fav_dists: dict[str, dict[str, np.ndarray]] = {}
    disclosed: dict[str, ArmResult] = {}
    for obj in OBJECTS:
        cond = cond_masks[obj]
        ...
        obj_fav: dict[str, np.ndarray] = {}
        for variant in VARIANTS:
            arm, fav_dist, _ = signal_arm(obj, variant, ohlc, ma, cond, entry_idx,
                                          entry_epoch, cell_index)
            signals[variant] = arm
            obj_fav[variant] = fav_dist
        ...
        fav_dists[obj] = obj_fav
        # Disclosed in-progress VP-POC: signal arm only (no null / no paired; never binding).
        d_arm, _, _ = signal_arm(obj, DISCLOSED_SPEC.vid, ohlc, ma, cond, entry_idx,
                                 entry_epoch, cell_index)
        disclosed[obj] = d_arm
    return arms, fav_dists, disclosed
...
def _causality_ok(
    ...
    if valid.any():
        ...
        # VP prior-completed reference span end (= MA segment end) is at/before entry.
...
def _cell_invariants(arms_obj: dict[str, Any], fav_dists: dict[str, np.ndarray]) -> dict[str, bool]:
    """Predeclared structural invariants for one object (scope §Success/Failure (iii)-(vi))."""
    ...
    # (iii) matched-count holds: each null's draw target == its variant's signal qualifying m.
    matched_ok = all(nulls[v].draw_count == signals[v].m for v in VARIANTS)
    # (vi) fav_dist > 0 for every counted event (validity rule): a counted event is in the
    # qualifying mask, which is a subset of the built-barrier ok mask (fav_dist > 0).
    fav_pos = True
    for v in VARIANTS:
        q = signals[v].qual
        fd = fav_dists[v]
        if q.shape[0] and fd.shape[0] == q.shape[0]:
            if bool((q & ~(fd > 0.0)).any()):
                fav_pos = False
                break
    ...
    for obj in OBJECTS:
        ...
        for variant in VARIANTS:
            ...
            rows.append({
                ...
                "variant": variant, "fav_model": FAV_MODEL[variant],
    ...
    for obj in OBJECTS:
        for variant in VARIANTS:
            rows.append({
                ...
                "fav_model": FAV_MODEL[variant], "is_benchmark": variant == "BENCH",
...
def composition_readout(rows: list[dict[str, Any]], defect: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "binding_lever_set": ("alternative favourable targets: /VPTARGET (POC, near-VA, far-VA "
                              "of the prior completed MA segment), /MAGTARGET (frac 0.5/1.0 x W 5/20 "
                              "of trailing MA-segment magnitudes) on MA(20,50)"),
        "reference": "BENCH = 0.50*M_sofar (MA-defined; reconciles EXP-061 M0/H0)",
        "disclosed": "VP-POC-INPROG (in-progress MA-segment POC; never binding)",
...
def _rule_text() -> str:
    return ("Per object, never pooled. Binding endpoint = median (P14); mean + 10% trimmed mean + "
            ...
            "alternative favourable variant variant_wins composes (P11 + P6 non-4h: >=5 cells / >=3 "
            "instruments / >=3 cells outside 4h). EVIDENCE_AGAINST iff the powered grid composes but "
            "no alternative variant wins. INCONCLUSIVE iff <P11 quorum powered. "
...
def _routing_text() -> str:
    return ("Feeds the single terminal G-015 after the full Phase 015 slate (no closure or "
            ...
            "favourable-target geometry is an MA-substrate lever -> the winning variant + its RM "
            "and benchmark margins feed EXP-067 (hybrid) / EXP-068 (native) / G-015. "
            "EVIDENCE_AGAINST: as on ZigZag (EXP-056 0/8), the favourable target is not the lever "
            "on MA for that object; family stays OPEN, the surface (S2/S3/S4) runs regardless (P9).")
...
def plot_contrast_heatmap(rows: list[dict[str, Any]], save_path: Path) -> None:
    ...
    fig.suptitle(f"{EXPERIMENT_ID}: favourable-target lever + signal attribution (per object)")
    fig.tight_layout()
    fig.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def plot_median_vs_mean(rows: list[dict[str, Any]], save_path: Path) -> None:
    """P4 skew preview: per-object median vs raw vs trimmed mean for BENCH + best alt variant."""
    fig, axes = plt.subplots(len(OBJECTS), 2, figsize=(14, 9), sharey=True)
    for oi, obj in enumerate(OBJECTS):
        best = _best_alt_variant(rows, obj)
        panels = [("BENCH", "benchmark 0.50*M_sofar"), (best, f"best alt: {best}")]
...
def secondary_rows(instrument: str, cell: dict[str, Any]) -> list[dict[str, Any]]:
    """Disclosed-secondary rows: the in-progress VP-POC arm per object (never binding)."""
    rows = []
    for obj in OBJECTS:
        d = cell["disclosed"][obj]
        rows.append({
            "instrument": instrument, "domain": cell["domain"], "object": obj,
            "variant": DISCLOSED_SPEC.vid, "fav_model": FAV_MODEL[DISCLOSED_SPEC.vid],
            "m": d.m, "median": d.median, "ci_low_1s": d.ci_low_1s, "mean": d.mean,
            "trimmed_mean": d.trimmed_mean, "tail_share_worst5": d.tail_share_worst5,
            "r_firsthit": d.r_firsthit, "win_rate": d.win_rate, "data_censored": d.data_censored,
            "median_viable": _viable(d), "tickvol_proxy": TICKVOL_PROXY,
        })
    return rows
...
def write_outputs(
    ...
    """Persist the per-cell parquet, the favourable / attribution maps, and JSON."""
    pl.DataFrame(rows, strict=False).write_parquet(RESULTS_DIR / "per_cell_expectancy.parquet")
    member = [r for r in rows if not r.get("excluded")]
    fav_cols = ["instrument", "domain", "object", "variant", "fav_model", "is_benchmark", "m",
                ...
                "paired_n_common", "data_censored", "low_n_4h", "r_firsthit", "win_rate"]
    _write_csv([{k: r.get(k) for k in fav_cols} for r in member],
               RESULTS_DIR / "favourable_target_map.csv")
...
def _write_metadata(
    ...
    meta = {
        ...
        "phase": "015", "surface": "S1", "hypothesis": "HYP-017", "family": "CF-HA-HARAMI-001",
        "checkpoint": "2026-06-17-015-ma-substrate-conditioned-harami-full-surface",
        "amendment": "D0-amendment-001-dual-parallel-substrate (2026-06-17); supersedes prior EXP-064 scope",
        ...
        "variants": {v: FAV_MODEL[v] for v in (*VARIANTS, DISCLOSED_SPEC.vid)},
        "binding_lever_set": list(ALT_VARIANTS),
        "reference": "BENCH = 0.50*M_sofar (MA-defined; reconciles EXP-061 M0/H0)",
        "disclosed_secondary_variant": DISCLOSED_SPEC.vid,
        "adverse_held_at_benchmark": "adv = C - rd*fav_dist (benchmark 1:1) for every variant",
        "third_barrier": "MA-defined adaptive cap (k=1.5, window=20, floor=6, median, min_moves=5)",
        "vp_construction": ("/VPTARGET volume profile of the prior completed MA segment "
                            "(LOOKBACK=1, [start_idx_k .. end_idx_k]); bin width 0.10*ATR_entry; "
                            "70% value area; POC / near-VA / far-VA; <3-bar reference span = "
                            "insufficient-profile (excluded-with-record). Causal: all reference "
                            "bars <= entry. " + TICKVOL_PROXY),
        "mag_construction": ("/MAGTARGET fav_dist = frac * median(trailing W MA-segment magnitudes "
                             "confirmed strictly before the harami); frac in {0.5,1.0}, W in {5,20}; "
                             "<W segments = warmup-excluded."),
        "params": {
            ...
            "vp_bin_fraction": VP_BIN_FRACTION, "vp_value_area": VP_VALUE_AREA,
            "vp_min_ref_bars": VP_MIN_REF_BARS, "mag_grid_frac": [0.5, 1.0],
            "mag_grid_window": [5, 20], "timecap_floor_bench": 6, "timecap_k": 1.5,
        ...
        "invariant_gates": ("per object: each arm's exit-reason weights sum to 1.0 (finite "
                            ...
                            "== its variant's signal qualifying m); fav_dist > 0 for every counted "
                            "event (validity). Reconciliation (SUBSTRATE/METHOD_DEFECT): native "
                            "BENCH reproduces EXP-061 M0 and hybrid BENCH reproduces EXP-061 H0 "
                            "per-cell m + median to 1e-9; a missing anchor is a defect."),
        ...
        "disclosed_secondaries_not_computed": (
            "With the favourable axis now run on two conditioning objects (16 binding variant arms "
            "+ their nulls per cell), the /STRONG-HA conditioning arm and the full ZigZag-substrate "
            "favourable surface (including the single ZigZag benchmark contrast vs EXP-056) are NOT "
            "computed here (runtime/budget); this read focuses on the binding 8-variant dual-object "
            "MA favourable axis + the P4 mean diagnostic. The disclosed in-progress VP-POC arm IS "
            "computed (secondary_map.csv). First-hit r, win rate, and exit composition are emitted "
            "per variant per object. If G-015 needs the deferred arms, they are a bounded follow-up."),
        "is_defect": defect["is_defect"],
        "de30_disclosure": DE30_DISCLOSURE,
        "tickvol_proxy": TICKVOL_PROXY,
        ...
        "registry": ("CF-HA-HARAMI-001/HYP-017 (EXP-064); composes the registered MA-SUBSTRATE "
                     "(hybrid + native modes), the benchmark favourable/cap geometry, the registered "
                     "/VPTARGET and /MAGTARGET branches (MA reuse, both objects), and the per-object "
                     "matched-random baselines (nulls). 0 candidate slots, 0 TEST reads; "
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=f"{EXPERIMENT_ID} MA favourable-target geometry (dual-object)")
    parser.add_argument(
