...
Runs the concluded strict + ratified-loose/fallback + incremental suite on the
...
EXP015_META = _results("EXP-015") / "run_metadata.json"
EXP015_DOMAIN_MDE = _results("EXP-015") / "domain_mde_summary.csv"
...
def dependency_manifest() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    checks = [
        ...
        ("EXP-015 metadata", EXP015_META, "COMPLETE"),
    ]
    ...
    for label, path in (
        ...
        ("EXP-015 domain MDE summary", EXP015_DOMAIN_MDE),
    ):
        ...
        blockers.append(
            {
                ...
                "reason": (
                    "dogfood reference book is undefined; EXP-016 scope forbids "
...
def load_suite_manifest(strict_mde_map: dict[str, float]) -> list[dict[str, Any]]:
    """Assemble per-domain strict/loose/fallback/incremental suite settings."""
    adoption = pl.read_csv(EXP012_ADOPTION)
    exp012_mde = pl.read_csv(EXP012_MDE)
    exp015_mde = pl.read_csv(EXP015_DOMAIN_MDE)
    manifest: list[dict[str, Any]] = []
    for domain in DOMAINS:
        ...
        inc_row = exp015_mde.filter(pl.col("domain") == domain).row(0, named=True)
        ...
        if not math.isfinite(inc_mde):
            raise RuntimeError(f"EXP-015 domain MDE is not finite for {domain}")
        manifest.append(
            {
                "domain": domain,
...
def run_dogfood_path(
    suite_rows: list[dict[str, Any]], reference_book: pl.DataFrame
    ...
    for path in tqdm(files, desc="EXP-016 dogfood"):
...
# --------------------------------------------------------------------------- #
def main() -> None:
    """Run the EXP-016 suite composition anchor and write outputs."""
    configure_logging()
    ensure_output_dirs()
    manifest_rows, blocker_rows = dependency_manifest()
    if blocker_rows:
        write_blocked_metadata(manifest_rows, blocker_rows)
        LOGGER.warning("EXP-016 BLOCKED: %s", blocker_rows)
        return
    ...
    dogfood_standalone, dogfood_incremental = run_dogfood_path(suite_rows, reference_book)
    ...
    LOGGER.info("EXP-016 complete: %s", overall_status)
