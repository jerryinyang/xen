"""EXP-056 — Favourable-Target Geometry (Conditioned HA Harami).

``CF-HA-HARAMI-001`` / HYP-009 (Phase 014-B surface read 1). TRAIN-only, gross;
...
1. slices the first-49% (TRAIN) 1-minute rows by file-order prefix (TEST and the
   ...
   others min_coverage=0.90, carrying TickVolume) and fences to the TRAIN edge;
...
3. varies **only the favourable target** over a predeclared sweep of 8 binding
   variants (``BENCH`` 50%; ``/VPTARGET`` POC / near-VA / far-VA of the prior
   completed move; ``/MAGTARGET`` {0.5,1.0}x trailing-{5,20}) plus the disclosed
   ``VP-POC-INPROG``; the adverse target (1:1), the third barrier (adaptive cap),
   and the P15 path-ordered intrabar fills are held at benchmark;
4. computes per-cell **median** ATR-normalised gross expectancy with the
   ...
   readout; and
5. runs a determinism replay, a BENCH reconciliation anchor, and a cross-check of
   the BENCH stat arm against EXP-053's recorded benchmark as
   SUBSTRATE/METHOD_DEFECT guards.

Real prices throughout; HA prices enter only the harami/impulse detectors and
never any metric. TickVolume is a broker tick-count proxy (disclosed). Outputs
under results/ and plots/; created in orchestration.
"""
...
TICKVOL_PROXY = ("TickVolume is broker tick count, a proxy for traded volume; every "
                 "/VPTARGET result carries this proxy limitation (family spec).")
LOGGER = logging.getLogger(EXPERIMENT_ID)


# --------------------------------------------------------------------------- #
# Variant specification (predeclared favourable-target sweep)
# --------------------------------------------------------------------------- #
...
DISCLOSED_VARIANT = VariantSpec("VP-POC-INPROG", 8, "vp", vp_level="poc", vp_inprog=True)
ALL_VARIANTS: list[VariantSpec] = BINDING_VARIANTS + [DISCLOSED_VARIANT]
...
PB_INPROG, PB_INPROG_PC = 11000, 12000                  # disclosed in-progress VP-POC
...
def ma_segment_moves(ohlc: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    crossover stretch (from the first defined non-zero MA-diff sign up to the first
    sign flip) is excluded by construction — a crossover-to-crossover segmentation
    cannot anchor its start on MA warmup. This is a disclosed secondary baseline;
...
# --------------------------------------------------------------------------- #
# Pure computation — per-event volume-profile levels (/VPTARGET)
# --------------------------------------------------------------------------- #
def vp_levels_per_event(
    entry_idx: np.ndarray, state: InProgressState, mv: dict[str, np.ndarray],
    ohlc: dict[str, np.ndarray], atr_entry: np.ndarray, in_progress: bool,
) -> dict[str, np.ndarray]:
    """POC / VAL / VAH per entry from the reference move's real bars + TickVolume.

    Reference span: the prior **completed** move ``M_k`` (``[start_idx_k,
    end_idx_k]``, causal: all bars ``<= EndTime_k < ConfirmTime_k <= t_i``) or, for
    the disclosed in-progress variant, ``(end_idx_k, entry_idx]``. Spans with
    ``< VP_MIN_REF_BARS`` real bars are insufficient-profile (disclosed).
    """
    ...
    for e in range(n):
        ...
        if k < 0:
            continue
        if in_progress:
            a, b = int(mv["end_idx"][k]) + 1, int(entry_idx[e])
        else:
            a, b = int(mv["start_idx"][k]), int(mv["end_idx"][k])
        if b < a:
            continue
...
# --------------------------------------------------------------------------- #
# Pure computation — per-variant favourable/adverse targets + exclusion reasons
# --------------------------------------------------------------------------- #
def build_variant_targets(
    entry_idx: np.ndarray, entry_epoch: np.ndarray, entry_close: np.ndarray,
    state: InProgressState, mv: dict[str, np.ndarray], ohlc: dict[str, np.ndarray],
    atr_entry: np.ndarray, variants: list[VariantSpec],
) -> dict[str, dict[str, np.ndarray]]:
    """Build ``(fav, adv, ok, reason_*)`` arrays for each requested variant.

    ``ok`` marks events whose variant target is built and lies on the reversal
    side of ``C`` (``fav_dist > 0``). The mutually-exclusive ``reason_validity`` /
    ``reason_profile`` / ``reason_warmup`` masks record *why* an otherwise
    in-progress event is excluded (disclosed counts; never silently clamped).
    """
    rd = state.rd
    vp_cache: dict[bool, dict[str, np.ndarray]] = {}
    out: dict[str, dict[str, np.ndarray]] = {}
    for v in variants:
        if v.kind == "bench":
            out[v.vid] = _bench_targets(entry_close, rd, state.m_sofar)
        elif v.kind == "mag":
            out[v.vid] = _mag_targets(entry_epoch, entry_close, rd, mv,
                                      int(v.mag_window), float(v.mag_frac))
        else:                                              # vp
            if v.vp_inprog not in vp_cache:
                vp_cache[v.vp_inprog] = vp_levels_per_event(
                    entry_idx, state, mv, ohlc, atr_entry, v.vp_inprog)
            out[v.vid] = _vp_targets(entry_close, rd, vp_cache[v.vp_inprog], v.vp_level)
    return out
...
def _bench_targets(
    ...
    """Benchmark 50%-of-M_sofar favourable target (reproduces EXP-053 exactly)."""
    bar = benchmark_barriers(entry_close, rd, m_sofar)
    ok = bar["fav_dist"] > 0.0
    res = _zero_reasons(int(ok.shape[0]))
    res["reason_validity"] = ~ok
    return {"fav": bar["fav"], "adv": bar["adv"], "ok": ok, **res}


def _mag_targets(
    entry_epoch: np.ndarray, entry_close: np.ndarray, rd: np.ndarray,
    mv: dict[str, np.ndarray], window: int, frac: float,
) -> dict[str, np.ndarray]:
    """Trailing-magnitude favourable distance (``/MAGTARGET``)."""
    fav_dist, defined = trailing_magnitude_distance(
        entry_epoch, mv["confirm_epoch"], mv["magnitude"], window, frac)
...
def _pick_va_edge(
    entry_close: np.ndarray, rd: np.ndarray, val: np.ndarray, vah: np.ndarray,
    level: str,
) -> np.ndarray:
...
def matched_random_variant(
    ...
    targets = build_variant_targets(drawn, ohlc["epoch"][drawn], ohlc["close"][drawn],
                                    sub, mv, ohlc, atr_all[drawn], [variant])[variant.vid]
    return resolve_targets(ohlc, drawn, ohlc["close"][drawn], sub.rd, targets["fav"],
                           targets["adv"], n_event_all[drawn], atr_all[drawn],
                           targets["ok"], _rng(cell_index, PB_RAND_BOOT + variant.idx))
...
def ma_seg_variant(
    ...
    targets = build_variant_targets(entry_idx, entry_epoch, entry_close, state, seg,
                                    ohlc, atr_entry, [variant])[variant.vid]
    population = buildable & stat["retained_p75"] & targets["ok"]
    return resolve_targets(ohlc, entry_idx, entry_close, state.rd, targets["fav"],
                           targets["adv"], n_event, atr_entry, population,
...
def cell_causality_ok(
    ...
    """Runtime causality/invariant checks predeclared in analysis-plan Step 9.

    Returns ``False`` (recorded; ``>=3`` instruments -> SUBSTRATE/METHOD_DEFECT) if
    the domain-bar grid is not strictly increasing (duplicate/disordered CloseTime),
    or any VP reference move ``M_k`` is not strictly causal
    (``EndTime_k < t_i``; the in-progress span upper bar is ``<= t_i``).
    """
    epoch = ohlc["epoch"]
    ...
    if valid.any():
        kk = state.k[valid]
        # completed reference move ends strictly before the entry (EndTime_k < t_i)
        if not bool(np.all(mv["end_epoch"][kk] < entry_epoch[valid])):
            return False
        # in-progress span upper bar is the entry bar itself (<= t_i)
...
def compute_cell(
    ...
    targets = build_variant_targets(entry_idx, entry_epoch, entry_close, state, mv,
                                    ohlc, atr_entry, ALL_VARIANTS)
...
def _resolve_all_arms(
    ...
    inprog = resolve(stat["retained_p75"], DISCLOSED_VARIANT, PB_INPROG)
    out["stat"]["variants"][DISCLOSED_VARIANT.vid] = inprog
    out["stat"]["contrast"][DISCLOSED_VARIANT.vid] = _single_paired(
        entry_idx, inprog, out["stat"]["variants"]["BENCH"], cell_index, PB_INPROG_PC)
    return out
...
def _variant_record(cell: dict[str, Any], v: VariantSpec, bench_m: int) -> dict[str, Any]:
    ...
    rec["excl_profile"] = int((cond & targets["reason_profile"]).sum())
...
def _empty_variant_fields() -> dict[str, Any]:
    rec = {"n_buildable": 0, "n_conditioned": 0, "retained_fraction": None,
           "excl_validity": 0, "excl_profile": 0, "excl_warmup": 0,
...
def secondary_records(instrument: str, cell: dict[str, Any]) -> list[dict[str, Any]]:
    """Disclosed-arm rows: /STRONG-HA, STAT-MAD, and the in-progress VP-POC variant."""
    if cell.get("empty", False):
        return []
    rows = []
    ...
    rows.append(_secondary_row(instrument, cell, "stat", DISCLOSED_VARIANT.vid))
    return rows
...
def _secondary_row(instrument: str, cell: dict[str, Any], arm: str, vid: str) -> dict[str, Any]:
    ...
    return {"instrument": instrument, "domain": cell["domain"], "arm": arm, "variant": vid,
            ...
            "ci_lo_2s": a.ci_lo_2s, "ci_hi_2s": a.ci_hi_2s,
...
def _variant_composition(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "powered": _tally(powered), "viable": _tally(viable, with_cells=True),
        "win": _win_tally(win),
    }
...
def plot_variant_forest(records: list[dict[str, Any]], save_path: Path) -> None:
    ...
    fig, axes = plt.subplots(2, 4, figsize=(18, 11), sharex=True)
...
def plot_return_distributions(pooled: dict[str, list[float]], save_path: Path) -> None:
    ...
    ax.set_xticks(range(1, len(variants) + 1), variants, rotation=30, fontsize=8)
...
def plot_win_composition(records: list[dict[str, Any]], save_path: Path) -> None:
    """Variant x cell composition status grid (WIN / viable / spans-0 / power / excluded)."""
    ...
    ax.set_yticks(range(len(BINDING_VARIANTS)), [v.vid for v in BINDING_VARIANTS], fontsize=7)
    ax.set_xticks(range(len(cells)), cells, rotation=90, fontsize=3)
    ax.set_title(f"{EXPERIMENT_ID}: wins-over-benchmark composition (P11: 5 cells / 3 instr)")
    handles = [plt.Rectangle((0, 0), 1, 1, color=VSTATUS_COLORS[c]) for c in VSTATUS_CODES.values()]
    ax.legend(handles, list(VSTATUS_CODES.keys()), bbox_to_anchor=(1.01, 1),
...
def plot_exclusion_fractions(records: list[dict[str, Any]], save_path: Path) -> None:
    ...
    for i, vid in enumerate(variants):
        ...
        profile[i] = sum(r.get("excl_profile", 0) or 0 for r in rows)
    ...
    ax1.bar(x, validity, bottom=qualifying, label="excl: validity", color="#f46d43")
    ax1.bar(x, profile, bottom=qualifying + validity, label="excl: profile", color="#fdae61")
    ax1.bar(x, warmup, bottom=qualifying + validity + profile, label="excl: warmup",
            color="#cccccc")
    ax1.set_xticks(x, variants, rotation=30, fontsize=7)
    ...
    ax2.set_xticks(x, variants, rotation=30, fontsize=7)
    ax2.set_ylabel("median per-cell qualifying events")
    ax2.set_title("per-cell power by variant")
    ax2.legend(fontsize=7)
    fig.suptitle(f"{EXPERIMENT_ID}: qualifying / exclusion accounting by favourable-target variant")
    fig.tight_layout()
    fig.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def make_plots(records: list[dict[str, Any]], pooled: dict[str, list[float]]) -> None:
    ...
    plot_win_composition(records, PLOTS_DIR / "wins_over_benchmark_composition.png")
...
def run() -> dict[str, Any]:
    ...
    for instrument in tqdm(INSTRUMENTS, desc="instruments"):
        ...
        for domain in DOMAINS:
            ...
            if not cell.get("empty") and not cell.get("causality_ok", True):
                defect["causality_violations"].append(f"{instrument}-{domain}")
            _maybe_guard(train_1m, domain, meta, ci, cell, defect, instrument, replayed)
            del cell
        del train_1m

    # EXP-053 BENCH reconciliation: any mismatch is a SUBSTRATE/METHOD_DEFECT and
    # must gate the verdict before composition (analysis-plan Step 9).
    ...
    # Binding EXP-053 anchor: expected but unavailable, or zero cells actually
    # checked, is itself a SUBSTRATE/METHOD_DEFECT (never a silent pass).
    ...
    if not defect["exp053_available"] or defect["exp053_checked_cells"] == 0:
        defect["is_defect"] = True
    # Causality / invariant violations on >=3 instruments -> defect (Step 9).
...
def write_outputs(
    ...
    pl.DataFrame(records, strict=False).write_csv(RESULTS_DIR / "favourable_target_map.csv")
...
def _write_metadata(
    ...
    meta = {
        ...
        "phase": "014-B", "hypothesis": "HYP-009", "family": "CF-HA-HARAMI-001",
        ...
        "geometry": "OAT on favourable target; adverse 1:1 + adaptive cap + P15 held at benchmark",
        "variants_binding": [v.vid for v in BINDING_VARIANTS],
        "variant_disclosed": DISCLOSED_VARIANT.vid,
        "params": {
            ...
            "vp_bin_fraction": VP_BIN_FRACTION, "vp_value_area": 0.70,
            "vp_min_ref_bars": VP_MIN_REF_BARS, "mag_grid": "{0.5,1.0} x {5,20}",
        ...
        "paired_contrast_conditioning": ("beats_bench is a paired contrast on the "
                                         "common qualifying subset S (res.qual & "
                                         "bench.qual); compare contrast_bench_n=|S| "
                                         "against each arm's m and the per-variant "
                                         "DATA_CENSORED counts — variants with |S| "
                                         "materially below their own m are "
                                         "conditioning-sensitive (interpret with care)."),
        "is_defect": defect["is_defect"],
        "de30_disclosure": DE30_DISCLOSURE,
        "tickvolume_proxy": TICKVOL_PROXY,
        ...
        "registry": ("CF-HA-HARAMI-001/HYP-009 (EXP-056); exercises /VPTARGET + /MAGTARGET; "
