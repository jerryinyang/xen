...
Measures the incremental referee's portfolio-fitness MDE under the predeclared
reference/candidate dependence grid. The implementation uses only the first 70%
...
EXP014_META = _results("EXP-014") / "run_metadata.json"
...
N_WORKERS = max(1, int(os.environ.get("EXP015_WORKERS", os.cpu_count() or 1)))
TASK_CHUNKSIZE = 8
...
def dependency_manifest() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    add_json("EXP-014", EXP014_META, "PASS")
...
def write_blocked_metadata(
    ...
    write_json(
        RESULTS_DIR / "run_metadata.json",
        {
            ...
            "reason": "Required upstream Track B artifacts are not approved/present.",
...
def _evaluate_task(task: Task) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for alpha in ALPHA_GRID:
        row = incremental_gate_row(core, alpha=float(alpha))
...
def build_cells() -> tuple[dict[tuple[str, str], dict[str, Any]], list[dict[str, Any]]]:
    ...
    for path in tqdm(files, desc="EXP-015 load/domains"):
...
def run_tasks(
    ...
    if N_WORKERS == 1:
        ...
        for c_rows, v_rows in tqdm(iterator, total=len(tasks), desc="EXP-015 draws"):
...
# The five incremental-referee legs, retained per draw for failure attribution (F03).
LEG_COLS: list[str] = [
    "L1_readiness",
    "L2_significance",
...
# --------------------------------------------------------------------------- #
def main() -> None:
    """Run EXP-015 and write draw-level, summary, and plot artifacts."""
    configure_logging()
    ensure_output_dirs()
    manifest_rows, blocker_rows = dependency_manifest()
    if blocker_rows:
        write_blocked_metadata(manifest_rows, blocker_rows)
        LOGGER.warning("EXP-015 BLOCKED: %s", blocker_rows)
        return
    ...
    if not tasks:
        raise RuntimeError("No EXP-015 draw tasks were enumerated")

    LOGGER.info("EXP-015 tasks: %s | workers: %s", len(tasks), N_WORKERS)
    ...
    any_refuted = any(row["status"] == "REFUTED" for row in domain_rows)
    any_supported = any(str(row["status"]).startswith("SUPPORTED") for row in domain_rows)
    overall_status = "REFUTED" if any_refuted else ("COMPLETE" if any_supported else "INCONCLUSIVE")
    ...
    LOGGER.info("EXP-015 complete: %s", overall_status)
