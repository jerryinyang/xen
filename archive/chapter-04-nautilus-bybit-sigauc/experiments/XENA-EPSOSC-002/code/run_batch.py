#!/usr/bin/env python3
"""XENA-EPSOSC-002 BacktestNode batch runner — contract v1 emissions.

002 vs 001: RET_ANCHOR-only grid (HYBRID/STRETCH dropped — runner asserts); emission
window is the SHIFTED TRAIN [2022-07-01, 2023-12-18] (fix #1), NOT the full fence.
Warmup: data starts exactly at TRAIN_START_002 ⇒ ≤ W×15m (≤48h) warmup loss at the
window head (disclosed, design §5). Strategy + features are byte-identical reuse of 001
(sha epsosc_strategy 7e0da041…, features ccc6084d…; design §15 "subset of existing
strategy"). Below this line the machinery is 001's, with the two changes above.
...
# Frozen shifted TRAIN window start (design §5). End = fence.train_end_utc.
TRAIN_START_002 = datetime(2022, 7, 1, tzinfo=timezone.utc)

# Smoke: short window inside the 002 shifted TRAIN
...
def run_param_group(
    ...
    *,
    ...
    # 002 grid is RET_ANCHOR-only (design §4.2); HYBRID/STRETCH must never emit here.
    bad = [c["candidate_id"] for c in cells if c.get("clear_policy") != "RET_ANCHOR"
           or c.get("object_type") != "VOLARM"]
    if bad:
        raise ValueError(f"002 is RET_ANCHOR/VOLARM-only; non-conforming cells: {bad[:3]}")
...
def main() -> int:
    ...
    if args.smoke:
        ...
        start, end = TRAIN_START_002, fence.train_end_utc  # 002 shifted window (fix #1)
        summaries = run_param_group(
            cells, fence, manifest, start=start, end=end, band=args.band
        )
    else:
