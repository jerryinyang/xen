...
© Professeur_X). Implemented in **chart-timeframe mode only**: the indicator
timeframe equals the bar timeframe, so the Pine ``request.security`` /
``indexHighTF`` / ``indexCurrTF`` machinery collapses to identity
(``f_no_repaint_request`` returns its expression unchanged) and there is no
multitimeframe or real-time lookahead-offset semantics to reproduce. The port
therefore implements only the local EMA / Heiken-Ashi recursion.

Two source-fidelity hazards are implemented explicitly:
...
2. Pine ``ta.ema`` seeding. Pine seeds an EMA from the first available value;
   ...
   which the resulting state labels are identical — a discretion-free warmup
   rule.

No exported TradingView reference series is bundled, so callers may claim only
"deterministic re-implementation of the published Pine formula," not
Pine-equivalence.
...
must be computed on the aggregated real OHLC, never on these values.
"""
