...
# --------------------------------------------------------------------------- #
_VAL003_CODE = (
    PROJECT_ROOT / "archive" / "chapter-01-price-geometry-referee" / "experiments"
    / "VAL-003" / "code" / "run_experiment.py"
)
...
# --------------------------------------------------------------------------- #
# The 6 INFR-005 index symbols completing the Indices basket, by their RESOLVED
# broker strings (the collection filename tokens; see reports/infr005-resolved.txt).
# Canonical -> broker: EU50 -> STOXX50, GER40 -> DE40; the other 4 collected under
# their primary names.
TARGET_SYMBOLS = (
    "AUS200", "US30", "STOXX50", "DE40", "HK50", "UK100",
)
DROPPED_SYMBOLS = ()
...
# INFR-005 collection ran 2026-07-06. A target symbol's selected file (newest by
# the collected-timestamp token in its name) must have been collected on/after
# this date, else its INFR-005 collection is MISSING (no older files exist for
# these 6 new symbols, but the guard is retained from the VAL-005 lineage).
INFR003_MIN_COLLECTED = date(2026, 7, 5)
