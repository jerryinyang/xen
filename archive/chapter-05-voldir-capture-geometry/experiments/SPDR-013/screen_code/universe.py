"""Universe recompute + pin assertion (design §0.1 UNIVERSE-PIN / AMENDMENT-U1).

Ranks every readable catalog symbol by ``sum(close*volume)`` on fenced 1m bars over
``[train_end - 30d, train_end)`` (TRAIN band) and ASSERTS that the top-25 set equals the
family pin. A mismatch aborts the screen — the universe is never re-ranked after outcomes.
"""
...
def dollar_volume(symbol: str, lo_ns: int, hi_ns: int) -> tuple[float, int]:
    """``sum(close*volume)`` and bar count on ``[lo_ns, hi_ns)`` for one symbol."""
    d = symbol_dir(symbol)
    if not d.exists():
        return 0.0, 0
    total, n = 0.0, 0
...
def recompute_universe(manifest, *, progress: bool = True) -> dict:
    """Recompute the ranking over the whole readable catalog under the TRAIN fence."""
...
def assert_pin(recomputed: dict) -> dict:
    """Assert set equality against BOTH pin files (design §0.1 'code assert')."""
    pins = {}
