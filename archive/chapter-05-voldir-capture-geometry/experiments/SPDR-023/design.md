# SPDR-023 — Volatility-adaptive management after MR breach entries
...
How does each confirmed volatility component change the MR breach model's native `z/H` geometry
and each applicable external management device relative to their fixed forms?

```text
MECHANISM:
  E-TOUCH or E-CLOSE supplies a causal breach side and MR enters against that side at the next real
...
DERIVED:
  ...
  null     = same zone origins under fixed z/H; same MR episodes under the fixed device;
...
OBJECT-IDENTITY:
  ...
  measured conditioning event == traded entry event: YES — first E-TOUCH/E-CLOSE under the
    declared fixed/direct/reverse z/H rule, entry at the next real open against the breach side
...
The baseline enters against the breach side, uses unit size, has no target, stop or trail, and exits
after four H1 bars.
...
1. Centre `100`, unit width `1`, categorical `HIGH`: direct `z=1.0`, fixed `z=1.5`, reverse
   ...
   MR enters short at the next real open `101.4`.
2. Centre `100`, lower `98.5`: the first outside close `98.4` occurs on origin bar 16.
   `LONG_ON_HIGH H=24` produces a down E-CLOSE and next-open long entry `98.3`; fixed `H=12` and
   `SHORT_ON_HIGH H=4` expire without an event.
3. Short fill `100`, target `98`, stop `102`: minute path `99/98.8` then `99.5/97.8`.
   Nautilus exits at the target before any later stop touch.

## Claim boundary

This experiment describes MR only. It does not gate either companion experiment, choose between
