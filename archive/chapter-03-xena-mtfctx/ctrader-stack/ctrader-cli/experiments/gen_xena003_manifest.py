"""Generate the XENA-003 universe manifest (design.md §3 grid, 2,736 candidates).

Writes ``data/strategy_runs/XENA-003/universe_manifest.json`` in the
...
MtfCtxReversionModel emits — no timestamp suffix.
...
Usage:  python tools/ctrader-cli/experiments/gen_xena003_manifest.py
"""
...
OUT_DIR = ROOT / "data" / "strategy_runs" / "XENA-003"
...
def main() -> None:
        ...
        for pair in DOMAIN_PAIRS:
            for hold in HOLD_LABELS:
                for v in range(N_VARIANTS):
                    cid = f"C3-{symbol}-{pair}-{hold}-V{v:02d}"
                    candidates.append({
                        "candidate_id": cid,
                        "run_dir": cid.lower(),
                        "symbol": symbol,
    ...
    manifest = {
        "universe_id": "XENA-003",
        "family": "CF-MTFCTX-001",
        "model": "MtfCtxReversion (CTRL-03 NAIVE REVERSION, trailing 3-bar-extreme limits, native m1 fills)",
