#!/usr/bin/env python3
"""XENA-EPSOSC-001 stage-1 search + certify (CLS-EPISODE / INFR-015 pin).
...
def append_xena_runs_registration(*, n_cands: int, pin_sha: str, search_band: str) -> None:
    ...
    if "XENA-EPSOSC-001" in text and "search/certify in flight" in text:
        return
    if "| XENA-EPSOSC-001 |" in text:
        return
    row = (
        f"| XENA-EPSOSC-001 | {_utc_now()[:10]} | CF-EPSOSC-001 VOLARM binding "
        f"({n_cands}) | {pin_sha[:8]} | {search_band} | *(pending close)* | "
        f"*(pending)* | — | **0/2** | search/certify in flight (operator-approved "
        f"2026-07-17); no TEST gate | pending |\n"
    )
...
def main() -> int:
    ...
    # Stage-2 gate band: top-1 only (pin one_subset)
    stage2: dict[str, Any] = {"empty": True}
    if ranked_ser:
        top = frozenset(ranked_ser[0]["subset"])
        print(f"stage-2 LCB on gate band for top-1 |S|={len(top)}…")
