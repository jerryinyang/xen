...
Failed, non-primary, excursion-censored, confirmation-censored, endpoint-censored,
and missing-outcome rows remain in status/count tables and never enter the completed
outcome denominator. `swing_price`, `swing_bps`, and `swing_duration_ns` are read
...
TRIPWIRE: CONFIG_CROSSWISE_FUTURE_DESTROY
  ...
  live read: on the unresampled source population compute D_raw and
    ...
    abs(m_destroy[s]) <= INTEGRITY_Z*bootstrap_SE_mean_destroyed[s]. If the latter
...
```text
FIXTURE-TOPOLOGY:
  rows_per_arm: 200 (BASELINE and ARM); one row is one complete level cluster;
