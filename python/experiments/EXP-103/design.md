...
CONTROL GAP_CROSSWISE_FUTURE_DESTROY:
  ...
  mapping: copy the raw rows; sort each group by (raid_id, original_row_position).
    ...
    nullness, and row counts stay fixed. A group with n<2 stays fixed and is
    disclosed via the group-size report; it does not void the control, because
    its rows contribute identically to the raw and destroyed contrasts
    (AMENDMENT-16). The control voids only when no group is movable
