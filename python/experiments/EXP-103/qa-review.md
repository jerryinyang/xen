...
## QA run 9 — 2026-08-17T23:50:14Z — mode: subagent — HEAD 62983d0

Verdict: **REVISE**

Scope: fresh-context re-review of EXP-103 at HEAD 62983d0 (experiment code state = e57847c, "fix(EXP-101-104): implement exact nested destroy and registered fixture plants"; later commits add skill/QA docs only; working tree clean at review start). This run verifies that the QA run 8 REVISE findings (recorded at HEAD 8127c23) are resolved in the fix commit and that no new issues were introduced. Expectations were derived from design.md §1–§10 and the family registry (`docs/signal-registry/candidate-families/cf-liqswp-001.md`) before code; no implementation, design, test, or receipt file was modified. Supporting evidence: the permitted test subset ran green (55 passed) and the committed fixture receipt (`results/fixture_integrity.json`) was inspected.

### Design-fidelity trace

| Design clause (§ref) | Code (file:line) | Verdict | Notes |
|---|---|---|---|
| **AMENDMENT-15 live-read inequality** (§5/§6 TRIPWIRE live read; §9 ledger) | `destroy.py:818-946` (`future_destroy_attestation`), bite loop `destroy.py:880-898`; `adapter.py:404-410` | MATCHES | Per outer seed s with finite `bootstrap_se_raw`: when `abs(D_raw) > INTEGRITY_Z*bootstrap_se_raw[s]` (INTEGRITY_Z=2.8, destroy.py:49), survival is judged by `abs(m_destroy) <= INTEGRITY_Z*bootstrap_se_raw[s]`; a violating seed appends `VOID_FUTURE_DESTROY_SURVIVAL`. The comparison uses the RAW bootstrap SE (AMENDMENT-15's amended target), never the nested SE; `bootstrap_SE_mean_destroyed[s]` is still computed and disclosed per seed (`destroy.py:700-718`, receipt `nested_seeds`). Collapse ratio is NaN when raw is zero/non-finite; no-seed-bite → control reported with `raw_bite=false` and no collapse attestation claim; missing/non-finite statistics add `VOID_NONFINITE_RAW/DESTROY_STATISTIC`. |
| **Nested outer bootstrap (run-8 issue 1): per-seed 10,000 populations, destroy recomputed inside every population b** | `destroy.py:1-47` (docstring), `destroy.py:520-748` (`nested_destroy_bootstrap`, `_destroy_draw`); `adapter.py:400-403` (`block_length=5`, seeds 0..4, n_boot=10,000) | MATCHES (closed-form realization) | Per seed s=0..4, n_boot=10,000 joint level-cluster populations ([§4] mechanics: circular blocks, L_eff cap, joint resampling; EXP-103 `independent_arms=False`, `adapter.py:198-200,404`). For every population b the destroyed-contrast mean over the 2,000-draw derangement distribution and the exact within-draw variance are recomputed from b's own per-cluster per-group sufficient statistics — donor pool = b's rows (cluster-resample/destroy coupling). Independent external probe: the closed-form derangement mean `(W·G−S)/(m−1)` and the exact derangement-variance formula match brute-force enumeration of all derangements to ≤1e-14 across m=3..6 (random arm/comparator splits). SE is the Law-of-total-variance composition `sqrt(var_between + mean_b(Var_draw)/n_destroy)` — the expected value of the literal "std over 2,000 realized draw-means" statistic; this expectation-equivalence (not draw-per-draw sampling) is disclosed in the docstring and is a deliberate performance realization of the registered procedure. Not average-then-bootstrap. Single-cluster (`effective=1`) and `n_clusters==0` guard paths exist; `bootstrap_se_raw` and `bootstrap_se_mean_destroyed` per seed (`destroy.py:700-718`). |
| **Per-seed disclosure of both SEs** (§5/§6 outer bootstrap) | `destroy.py:700-718`; receipt `nested_seeds` | MATCHES | Every nested seed row exposes `bootstrap_se_raw` and `bootstrap_se_mean_destroyed` plus `var_between_populations`/`var_within_draws_over_n_destroy`; receipt shows all 5 seeds × 3 control channels (n_boot=10 smoke, per FIXTURE-TOPOLOGY). |
| **Registered two-arm fixture plants (run-8 issue 2)** | `adapter.py:73-172` (`make_fixture_frame`); `analysis.py:325-341` (`fixture_frame` joins golden-profile rows per arm) | MATCHES | 200 rows/arm, no nulls, identical profile labels/status/nullness except arm+outcomes; +0.50 ATR (alt 0.90/1.10 vs 1.40/1.60), +3.6e12 ns (3.0/4.2 vs 6.6/7.8 e12), +0.25 strong_move (¼ vs ½); `level_id=FIXTURE-{arm}-level-{i:04d}`; `first_raid_timestamp` base+step; deterministic permutation seed 4 then raid_id; ordering by `(sweep_ts_ns min/level, level_id)`; `config=FIXTURE_CONFIG` for both arms. Receipt raw estimates 0.5 / 3.6e12 / 0.25 confirm exact plants. |
| **Nullness class = declared five-bit tuple with `duration_ns` alias (run-8 issue 3)** | `adapter.py:38-46`; `analysis.py:42-46,51-57`; alias byte-equality `adapter.py:338-354` | MATCHES | `CONTROL_NULL_COLUMNS = (swing_price, swing_bps, swing_atr, duration_ns, strong_move)` — the declared 5 bits with the `duration_ns` alias. Row-wise equality `duration_ns == swing_duration_ns` asserted before any read; mismatches add `VOID_DURATION_ALIAS`, nullness asymmetry adds `VOID_DURATION_ALIAS_NULLNESS_MISMATCH`. |
| **`swing_price`/`swing_bps` source summaries in `analyze()` output (run-8 issue 4)** | `adapter.py:466-561` (`source_field_summaries`) | MATCHES | Per-arm/comparator `n`, `non_null`, `mean`, `median` for both source channels on the eligible DEFINED population; contract test asserts presence in every value row. |
| **Empirical 95% destroyed interval in live control records (run-8 issue 5)** | `destroy.py:891-895` (`destroyed_interval` = 2.5%/97.5% quantiles of the 2,000 destroyed contrasts) | MATCHES | Present per control record (receipt: swing_atr [−0.054, 0.050], duration [−3.84e11, 3.6e11], strong_move [−0.09, 0.09]). All 2,000 destroyed contrasts disclosed (`destroyed_contrasts`). |
| **Fixture receipt regenerated and passing (all control records blocking_pass)** | `results/fixture_integrity.json` (committed at e57847c) | MATCHES | 3/3 control records `blocking_pass=true`, `reasons=[]`; `raw_bite_seeds=[0,1,2,3,4]` on all three channels; destroyed means (−0.00184 / −1.27e10 / −0.00078) fall inside `INTEGRITY_Z*bootstrap_se_raw` bands (e.g. 2.8×0.00428=0.0120 for swing_atr); collapse ratios ≈ −0.003 (receipt asserted near-zero per design); `nested_seeds` finite (n_boot=10 smoke); `population_match=true`; top-level and per-record `blocking_pass=true`; `value_rows` populated with the 5 channels (observed_L2/L5/L10, source_field_summaries). |
| **source.py repairs (run-8 residuals): composite uniqueness, UTC fence, cell-scoped duplicates** | `source.py:126-152` (`_validate_composite_uniqueness`), `source.py:157-178` (`_validate_utc_fence`), cell-scoped object-id duplicates `source.py:319-345` | MATCHES | Composite `(source_cell, raid_id)` uniqueness checked on an eager per-cell frame; `train_end_ns = 1_700_611_200 s = 2023-11-22T00:00:00Z` verified by parse (`TRAIN_END_UTC`); pinned fence receipts (`status=PINNED`, `train_end_utc` equality) enforced; within-cell duplicate raid_ids fail closed; run-8's earlier identity/row-count/causal-order checks retained; live-contract test passes on the actual 264-cell source (264 paths, blocking pass). |
| **Nested-destroy performance proof (run-8 residual)** | `python/tests/test_exp10x_nested_destroy_performance.py` | MATCHES | 4,000-row / 1,000-cluster representative stratum at live scale (5 seeds × 10,000 × 2,000) completes within the 120 s bound for joint and independent arms; both per-seed SEs finite. Ran green in this review. |
| **Derangement zero-fixed-points / non-vacuity** | `destroy.py:87-95` (`derange_indices` rejection sampling); `destroy.py:474-482` (`VOID_NO_CHANGED_VALUE`); groups <2 rows → `VOID_SINGLETON_GROUP` | MATCHES with note | `perm[i] != i` enforced on ever row; fixed_points always 0; non-vacuity confirmed on fixture (changed-values > 0, moved_eligible_values 400). Reason-code naming for n<2 groups is `VOID_SINGLETON_GROUP` rather than the design's literal `VOID_NO_DERANGEMENT` — same disclosed semantics (see Issue 3). |
| **Populations, estimand, fixed comparator (§3)** | `adapter.py:262-279` (`_channel_frame`: COMPLETED + primary_attribution + primary_completed + DEFINED profile), `analysis.py:263-268`; contrast True−False | MATCHES | Outcome denominator exactly the §3 population; non-tight defined profiles within `archive_symbol×timeframe×method×reference×side×config` are the fixed comparator; all-defined baseline is disclosure-only (`extra.all_defined_baseline`); ATR_UNDEFINED excluded from `swing_atr`/`strong_move` per the binding decision, with excluded counts visible in census. |
| **Left-join profile retention, `profile_generation` handling (§1 JOIN)** | `analysis.py:266-323` (`live_frame`, `profile_join_reason`, `VOID_DUPLICATE_PROFILE_KEY`, `VOID_PROFILE_JOIN_MISMATCH`) | MATCHES | Left join on `(source_cell, raid_id, profile_generation)`; unmatched raids are `MISSING_PROFILE`, extras and duplicate keys counted and fail closed; failed/censored/undefined rows stay visible (status census, undefined reasons). |
| **§4 estimator contract: joint clusters, empty arms, L=2/5/10, quantiles** | `statistics.py:112-176` (`clustered_contrast_bootstrap`), `statistics.py:335-356` (`block_sensitivity`), `adapter.py:466-561` | MATCHES | Clusters = complete `level_id` histories sorted by `(first_raid_timestamp, level_id)` (`adapter.py:288-297`); joint resampling default for EXP-103; L_eff cap `min(max(1,L), n_clusters−1)` (`statistics.py:117-119`); `EMPTY_ARM_OR_COMPARATOR` / `ONE_CLUSTER` / `NO_FINITE_DRAWS` disclosed with counts, never silently dropped; NumPy linear 2.5%/97.5% quantiles; per-seed bounds, median bounds, seed bounds range exposed; `observed_L2/L5/L10` first-class fields. |
| **§5 profile integrity checks (by cell and stratum)** | `analysis.py:93-152` (`replay_profile`), `analysis.py:174-238` (`validate_profile_frame`), `analysis.py:154-172` (`golden_profile_frame`), join evidence, census | DEVIATES (partial) | Implemented: one-to-one join counts, TPO conservation (`tpo_conservation_ok`), fixed bin assignment (`bin_width == 0.1×atr`), VA-mass internal consistency, strict `<0.50` tight boundary, positive VA_width (implicit via geometry), gap-mask binding + sha256 format, undefined-reason visibility, deterministic replay of the golden/fixture traces. NOT implemented: re-verification of the ≥70% VA-mass threshold and ≥30% gap-mass threshold from emitted fields, minimum-bin, zero-ATR, empty-profile-as-check, reset-on-new-maximum, and per-cell live deterministic replay (not recomputable from emitted profile fields; design §2 forbids reconstruction). Run-8 marked this row MATCHES; independent read finds a partial implementation (Issue 2). |
| **§1 seal/fence ("require" clause)** | `source.py:180-345` (`validate_source_contract`) | DEVIATES (partial) | Validated: family gate blocking/n_cells, per-cell gates, `config_hash` (metadata↔gate), `event_log_sha256` (metadata↔file), `cost_model=NO_COST_CHARGED`, fence `PINNED`, `train_end_utc`/`train_end_ns` (2023-11-22T00:00:00Z = 1,700,611,200 s). NOT validated: `emission_contract_version=nautilus-emission-v1`, `nautilus_version=1.230.0`, `one_backtest_node=true` (all present in `run_metadata.json`), the design's manifest SHA256 `4cdc7b01…`, and the TRAIN start fence `2021-06-02T00:01:00Z`. Content-addressed hashes already bind the source, so the practical risk is low, but the §1 "require" seal is not fully enforced (Issue 1). |
| **§1 gate-first** | `analysis.py:271-277`; `source.py:180-345`; `runtime.py:66-73` | MATCHES | Family gate + per-cell gates checked before any parquet outcome row is scanned; live test asserts 264 paths and blocking pass. |
| **Amendment ledger (§9)** | `design.md:202-232` | MATCHES | AMENDMENT-15 declared **LOOSER**; running count 3 looser / 3 tighter / 8 neutral; final accounting 3/3/8; no ≥3 one-directional streak; F02/F04/F06 N/A; F07 satisfied. |
| **Zero-cost disclosure (§10)** | `contract.py:10-25` (`ZERO_COST_DISCLOSURE`); receipt `zero_cost_disclosure` | MATCHES | Canonical text verbatim (cost_model NO_COST_CHARGED; spread/commissions/swaps not modeled; implication wording; prohibited_claims fully-net/cost-complete/tradable/deployable; lifting); contract test asserts receipt equality; no cost function on any live path. |
| **No research powering / PSR / battery rules** | grep over `liqswp_analysis` + analysis_code | MATCHES / N/A | No MDE, detection floor, `UNPOWERED`, power curve, `min_powered_seeds`, `n_legs` anywhere; `INTEGRITY_Z=2.8` used only as validity bite scale; no trade/leg-bps series → PSR N/A; no battery selection/capped read → L-24 N/A; report layers constrain interpretation (no SUPPORTED/WASH/CONTRADICTED fields). |

### Golden-trace diff

| Event | Expected from design (§8) | Implemented logic (hand-verified) | Verdict |
|---|---|---|---|
| T1 tight fixture, counts [29,12,23,23,27,26], bin_width 1, ATR 10 | POC=100 (lowest-price max-count); VA expands 100→101→102→103→104; va_count=114, target=ceil(0.70×140)=98; val=100, vah=105, VA_width=5; gap ≤30% of 114 → bins 101,102 (12+23=35→ceil(34.2)=35); gap_span=2, gap_span_va=0.40, tight_gap=true | `replay_profile` (analysis.py:93-152) reproduces: POC `max(range(len(counts)), key=(count,-index))` → 100; upper-first expansion loop → bins 100..104; gap selection sorted by `(count,index)` cumulative to 35 → bins 101,102; `tight_gap = span < 0.5×width` → true | MATCHES |
| T2 non-tight fixture, counts [10,18,13,7,7,30] | POC=105; lower-neighbour expansion 105→104→103→102→101; va_count=75, target=ceil(0.70×85)=60; val=101, vah=106, width=5; gap 30% of 75 → bins 103,104,102 (7+7+13=27≥23); gap_span=3, gap_span_va=0.60, tight=false | Same replay: POC=5→105; lower-side expansion at boundary (upper sentinel −1) → bins 1..5; gap → 103,104,102; span=3; 3 < 2.5 false | MATCHES (note: gap "outer" span counts bins high−low+1, per §2 +bin_width) |
| T3 strict boundary | gap_span=2, VA_width=4 → ratio 0.50 → tight=false (strict `<`) | `validate_profile_frame` VOID_TIGHT_GAP_BOUNDARY enforces `tight_gap == span < 0.5×width`; adapter test asserts the 0.50 boundary frame passes | MATCHES |
| Profile fixture expectations in the analysis fixture | T1/T2 golden rows replicated onto the two planted arms (tight→T1, non-tight→T2) with bound raid_id/generation/gap_mask | `analysis.py:325-341` (`fixture_frame`: golden templates enriched per row, masks bound, joined on raid_id) | MATCHES |
| Cross-check against the frozen emission producer | EXP-100 logic must equal the design's frozen profile contract | `exp100/tpo.py:215-301` (`finalize`): POC = strictly-greater replace (lowest max-count), `_value_area` upper-first on `>=` ties, target `0.70×total`, `_gap_span` uses `iter_profile_bins_by_density` sorted by `(count,bin)` = lowest-density first, `target=0.30×va_count`, `tight_gap = gap_span < 0.50×va_width` (`tpo.py:202`) | MATCHES (design ↔ emission producer ↔ analysis replay all agree) |

### Governance & boundary

- **Fresh context:** PASS — dedicated subagent; no implementation authorship in this conversation.
- **Gate-first:** PASS — family gate (blocking_pass=true, n_cells=264) and 264 per-cell gates before any source row read (live contract test green).
- **TRAIN/holdout:** PASS — `endpoint_ts_ns <= 1_700_611_200 s` fence enforced in scan and per-cell validation; `TRAIN_END_UTC` parity verified; no TEST path, no holdout access; no Python engine/backtest anywhere (analysis-only vehicle).
- **Registry:** PASS — `CF-LIQSWP-001/HYP-003` registered with EXP-103 mapping (cf-liqswp-001.md:121); design states no counted TEST reads; family gate checked.
- **No local accounting:** PASS — `check_no_local_accounting('python/experiments/EXP-103/analysis_code')` → `{'ok': True, 'banned_defs_found': []}`.
- **Zero cost:** PASS — disclosure verbatim (design §10 ↔ contract.py, receipt equality asserted); no live cost-path imports; retired cost stack not referenced.
- **No research powering:** PASS — grep clean; `INTEGRITY_Z=2.8` validity-bite only.
- **Derangement:** PASS — rejection sampling (`derange_indices`), zero fixed points; non-vacuity asserted; singleton groups disclosed (naming note, Issue 3).
- **One BacktestNode:** PASS — EXP-103 constructs no engine; source metadata attests one_backtest_node=true (note: not re-validated by the analysis, Issue 1).
- **Amendment ledger:** PASS — AMENDMENT-15 LOOSER; 3 looser / 3 tighter / 8 neutral; no streak ≥3 in the final gate set.
- **PSR / screen-pin / XENA:** N/A — no money or leg-bps series; no screen conversion; no XENA routing.
- **Battery/eligibility/null rules (L-24):** N/A — no battery selection, capped read, or exit-matched null battery beyond the registered destroy; small-n rows disclosed with counts (F07).

### Issues

1. **LOW-MEDIUM — §1 "seal/require" clause only partially enforced.**
   **Design:** §1 FROZEN-SOURCE `seal:` line requires `emission_contract_version=nautilus-emission-v1`, `Nautilus=1.230.0`, `cost_model=NO_COST_CHARGED`, and `one_backtest_node=true`, plus the fence includes the manifest SHA256 `4cdc7b01…` and a TRAIN start boundary `2021-06-02T00:01:00Z`.
   **Code:** `source.py:180-345` validates config_hash, event_log_sha256, cost_model, and the end-fence only. It never reads `emission_contract_version`, `nautilus_version`, `one_backtest_node` (all present in `run_metadata.json`), never checks the manifest SHA, and never validates the TRAIN start fence.
   **Required change:** Either implement the missing seal validations in `validate_source_contract`, or amend the design to state that these properties are attested by the EXP-100 gate (and are not re-verified by EXP-103) — then mark the trace row accordingly. Practical risk is low (config/event hashes content-bind the source), which is why this is not HIGH. **Route:** `experiment-developer` (code) or `quant-designer` (design clarification).

2. **LOW-MEDIUM — §5 "profile integrity checks" list only partially implemented in the analysis.**
   **Design:** §5 requires reporting (by cell and stratum) of: one-to-one join counts; TPO conservation; fixed bin assignment; POC tie-breaking; VA mass; 30% gap mass; strict 50% comparison; positive VA_width; minimum-bin, zero-ATR, empty-profile, undefined-reason, reset-on-new-maximum, and deterministic replay checks.
   **Code:** `validate_profile_frame` (analysis.py:174-238) implements the internal-consistency subset (masks, conservation flag, bin_width=A/10, VA-mass ratio, gap-span consistency, strict tight boundary, geometry). It does NOT re-verify the ≥70% VA-mass threshold, the ≥30% gap-mass threshold (both computable from emitted fields: `va_count/tpo_total`, gap selection vs `0.30×va_count`), minimum-bin, zero-ATR, empty-profile-as-check, reset-on-new-maximum, or per-cell live deterministic replay (last five not recomputable from the emitted profile table; design §2 forbids reconstruction).
   **Required change:** Add the computable threshold verifications (≥70% VA, ≥30% gap mass) to `validate_profile_frame`, and have the design clarify which processor-level checks (min-bin, zero-ATR, empty-profile, reset, replay) are EXP-100 gate obligations attested at EXP-100 rather than EXP-103 re-derivations. Run-8 marked this row MATCHES; the independent read found a partial implementation. **Route:** `experiment-developer` + `quant-designer`.

3. **LOW — Reason-code naming: `VOID_SINGLETON_GROUP` vs the design's `VOID_NO_DERANGEMENT`.**
   **Design:** §6 mapping: "A group with n<2 produces VOID_NO_DERANGEMENT and remains disclosed."
   **Code:** `destroy.py:236,371` emits `VOID_SINGLETON_GROUP` (and `VOID_NO_MOVABLE_ROWS` when nothing is movable). Semantics are identical and disclosed; no code or receipt depends on the literal `VOID_NO_DERANGEMENT` string. Note also that a stratum with a singleton destroy group is fail-closed in this implementation (reason propagates to a non-passing control), which matches the "failed derangement = invalidity" clause of §6. Cosmetic naming inconsistency only; align code or design text. **Route:** `experiment-developer`.

### Summary of run-8 issue resolution

Run-8 issues 1–5 and its residual risks are all **resolved** in e57847c: exact nested destroy (closed-form, performance-proven), registered two-arm fixture plants, `duration_ns` nullness alias with byte-equality assertion, `swing_price`/`swing_bps` source summaries, empirical 95% destroyed intervals, regenerated passing fixture receipt, source.py repairs, and the bounded-runtime proof. The golden trace and the AMENDMENT-15 acceptance inequality match the design text and the fixture receipt verifies every seed and channel. Two independent fidelity gaps (Issues 1–2) surfaced that run-8 had marked MATCHES; both are low/medium, pre-existing (not introduced by the fix commit), and have clear remediation paths. Per the governance framework (an unresolved DEVIATES/MISSING row in the trace → REVISE), the verdict is **REVISE** with two low/medium items and one cosmetic item, rather than APPROVE.

**REVISE.** Run-8 findings are all fixed and verified. Two low/medium design-to-code gaps remain from an independent clause-by-clause read (design §1 seal clause only partially enforced; design §5 profile-integrity check list only partially implemented), plus one cosmetic reason-code naming mismatch. Route to `experiment-developer` for the computable validations and `quant-designer` for the two design clarifications.

## QA run 10 — 2026-08-20T19:40:49Z — mode: subagent — HEAD 0f08bd13a68393be9d1f8344b69af6c6cff79f7b
Verdict: APPROVE

Scope: fresh-context pre-exec review of EXP-103 only (HYP-003 tight vs non-tight TPO gap). Analysis-only re-read of the frozen EXP-100 AMENDMENT-14 TRAIN emission (264 cells). No new engine, no TEST, no holdout, no EXP-100 rerun. Expectations derived from `python/experiments/EXP-103/design.md` §1–§10, checkpoint `2026-08-11-019-liquidity-sweeps` through AMENDMENT-16, then code. Prior on-disk QA (runs 1–9) treated as history, not evidence. Unofficial `analysis_results.json` not read. Wall-clock not queried (read-only, no shell); calendar date from the current handoff filename. Dirty/untracked ignored: `.jspace/`, `docs/superpowers/plans/2026-08-20-exp-101-104-handoff.md`, `python/experiments/EXP-10{1,2,3,4}/results/analysis_results.json`.

### Design-fidelity trace

| Design clause (§ref) | Code (file:line) | Verdict | Notes |
|---|---|---|---|
| Vehicle: analysis-only re-read of retained EXP-100 AMENDMENT-14 TRAIN; no new `BacktestNode`, TEST, or holdout (`design.md` header, §1) | `analysis.py:446-467` (`main`); `adapter.py:420-435` (`source_spec`); `runtime.py:109-118` (`run_live`) | MATCHES | Default live root is `data/nautilus_runs/EXP-100/full`. No strategy/runner under EXP-103. Cell gates read from EXP-100 `results/execution/full`. |
| §1 FROZEN-SOURCE gate-first, 264 cells, EXP-100 authority | `source.py:164-207` (`validate_source_contract`); `analysis.py:298-307`; `test_exp103_analysis_live.py:26-35` | MATCHES | Family gate `blocking_pass` + `n_cells==264` before any outcome row; per-cell gates reconciled; 264 paths required. Default `--gate` file is the EXP-103 copy (`results/estimand_validation.json`: `blocking_pass=true`, `n_cells=264`, `root=../data/nautilus_runs/EXP-100/full`). Copy/cell-gate disagreement fails `VOID_GATE_RECONCILIATION`. |
| §1 seal: `emission_contract_version=nautilus-emission-v1`, Nautilus `1.230.0`, `one_backtest_node=true`, `cost_model=NO_COST_CHARGED`, manifest SHA `4cdc7b01…`, TRAIN start `2021-06-02T00:01:00Z` through `2023-11-22T00:00:00Z` | `source.py:18-27` (pins); `source.py:256-278` (per-cell require); `source.py:157-161` (UTC end fence); `adapter.py:35-36,431-434`; `test_source.py:150-167` | MATCHES | Run-9 residual is closed. Fail-closed reasons: `VOID_EMISSION_CONTRACT_VERSION`, `VOID_NAUTILUS_VERSION`, `VOID_ONE_BACKTEST_NODE`, `VOID_ZERO_COST`, `VOID_MANIFEST_SHA256`, `VOID_TRAIN_START_FENCE`, `VOID_NAUTILUS_PIN`, `VOID_BEFORE_TRAIN_START`. `config_hash` compared metadata↔gate; `event_log_sha256` hashed from file. Fence `manifest_sha256` compared to the pinned constant; manifest file is then read for `analysis_start_utc` / `nautilus_pin` (file bytes are not re-hashed). |
| §1 JOIN: left join every raid; retain unmatched; null `profile_generation` must match | `analysis.py:328-349` (`live_frame`); `source.py:365-378` (`key_join_evidence`, `nulls_equal=True`); `test_source.py:278-300` | MATCHES | Live key is `(source_cell, raid_id, profile_generation)` with `how="left", nulls_equal=True`. ATR-undefined null generations count as matched. Unmatched → `MISSING_PROFILE`; extras/duplicates → `VOID_PROFILE_JOIN_MISMATCH` / `VOID_DUPLICATE_PROFILE_KEY`. Design names the within-cell pair `(raid_id, profile_generation)`; `source_cell` is required for composite identity across 264 cells. Helper `join_profiles_left` (`analysis.py:274-281`) still joins on `raid_id` only and is not the live path. |
| Live 1-hour observation cells labelled `60m` not `1h` | `source.py:112-116`; `exp100/processor.py:600`; `exp100/config.py:11` | MATCHES | Identity check builds `f"{observation_minutes}m"`; EXP-100 emits the same. `OBSERVATION_MINUTES` includes 60. A `1h` observation label would `VOID_ROW_IDENTITY_TIMEFRAME`. Confirmation references remain `1H`/`4H`. |
| §2 frozen profile contract; strict `gap_span < 0.50 * VA_width`; emitted fields only | `analysis.py:90-151` (`replay_profile`); `analysis.py:174-261` (`validate_profile_frame`); `analysis.py:243-244` | MATCHES | POC = lowest-price max-count; VA upper-neighbour first; gap lowest-density to ≥30% VA mass; `tight_gap = span < 0.5 * width`. Live path never rebuilds bins from later outcomes. |
| §3 outcome population = COMPLETED ∧ primary_attribution ∧ primary_completed ∧ DEFINED | `adapter.py:459-469`; `analysis.py:286-287` | MATCHES | Base mask is completed + both primary flags (`fill_null(False)`). EXP-103 then requires `profile_status==DEFINED`. Failed/censored/missing/undefined stay in census and do not enter the later-swing denominator. |
| §3 ATR_UNDEFINED exclusion on excursion channels only | `adapter.py:465-468`; `adapter.py:754-756` (census `atr_undefined`) | MATCHES | Extra filter only for `swing_atr` and `strong_move`. Finite non-excursion fields keep their excluded count beside them. |
| §3 contrast = tight minus non-tight; all-defined is disclosure only | `analysis.py:284` `contrasts=((True, False),)`; `statistics.py:29-51` (`arm_mean - comparator_mean`); `analysis.py:387-404`; `test_exp103_adapter.py:61-75` | MATCHES | Sole contrast pair is `(True, False)`. `extra.all_defined_baseline` is not an arm. Test asserts no other arm/comparator pair. Baseline is computed on all `DEFINED` rows (not re-masked to completed-primary); fixture rows are already that population. Primary estimand does not use this baseline. |
| §3 duration alias `swing_duration_ns == duration_ns` | `adapter.py:530-549`; `test_exp103_analysis_live.py:51-62` | MATCHES | Nullness XOR → `VOID_DURATION_ALIAS_NULLNESS_MISMATCH`; value mismatch → `VOID_DURATION_ALIAS`. Canonical duration is `swing_duration_ns`. |
| §4 joint level-cluster bootstrap, L=2/5/10, empty arm, linear quantiles | `adapter.py:389-392,480-496`; `statistics.py:56-72,112-176,335-356` | MATCHES | `independent_arms=False`. Clusters = complete `level_id` histories sorted by `(min(sweep_ts_ns), level_id)`. `L_eff=min(max(1,L), n_clusters-1)`. Empty arm → `EMPTY_ARM_OR_COMPARATOR` with counts, not dropped. NumPy default (linear) 2.5%/97.5%. |
| §5 computable profile integrity (fix-round clarification) | `analysis.py:174-261`; `test_exp103_adapter.py:9-57` | MATCHES | Implemented: 1:1 join, conservation, bin width `0.1×ATR`, VA-mass ratio, `va_count >= ceil(0.70×tpo_total)`, gap-span geometry, strict 50% boundary, gap mass where `gap_mass`/`gap_target_mass` exist (golden/fixture). Live table has masks only, so live gap mass is not recomputed (design §2). Min-bin / zero-ATR / empty-profile / reset / per-cell live replay remain EXP-100 gate obligations. Run-9 issue 2 closed. |
| §6 destroy groups; complete outcome block; derangement | `analysis.py:41-57`; `destroy.py:87-95,221-246,390-403`; `adapter.py:198-211` | MATCHES | Groups = symbol × timeframe × method × reference × side × config × status × primary_completed × five-bit nullness `(swing_price, swing_bps, swing_atr, duration_ns, strong_move)`. `derange_indices` rejects fixed points. Live destroy is per authorised channel on that grouping (equivalent for the channel statistic). |
| AMENDMENT-16: n<2 groups stay fixed; void only `VOID_NO_MOVABLE_ROWS` / `VOID_NO_CHANGED_VALUE` | `destroy.py:240-246,399-403,418-419,474-482` | MATCHES | Singletons are not moved and do not emit a void reason. Control voids only when nothing is movable or no eligible value changes. `group_sizes` disclosed. `VOID_SINGLETON_GROUP` is gone from `liqswp_analysis` (only a dummy string in `test_contract.py`). Run-9 issue 3 superseded. |
| AMENDMENT-15: destroyed non-bite vs raw SE; nested SE still disclosed | `destroy.py:55,981-1056,1099`; fixture receipt `raw_estimate` 0.5 / 3.6e12 / 0.25 | MATCHES | When `abs(D_raw) > 2.8 × bootstrap_SE_raw[s]`, require `abs(m_destroy) <= 2.8 × bootstrap_SE_raw[s]`. Survival → `VOID_FUTURE_DESTROY_SURVIVAL`. `bootstrap_SE_mean_destroyed[s]` computed and disclosed. Zero/non-finite raw → collapse NaN, no collapse claim. |
| §6 fixture plants + topology; smoke n_boot=10, live 10,000; 2,000 derangements | `adapter.py:75-178`; `analysis.py:351-367`; `runtime.py:104-107`; committed `results/fixture_integrity.json` | MATCHES | 200/arm, `FIXTURE_CONFIG`, permutation seed 4, timestamps/level_ids as specified, plants +0.50 ATR / +3.6e12 ns / +0.25 proportion. Receipt raw estimates match. Named `run_fixture()` defaults `n_boot=10`; live Adapter defaults 10,000. CLI `main()` builds a default Adapter (10,000) even for `--fixture` — slower smoke, not a live-path deviation. |
| §7 sample-size / no powering / PSR N/A / complexity | grep over `liqswp_analysis` + `analysis_code` | MATCHES | No MDE, detection floor, `UNPOWERED`, power curve, `n_legs_floor`. `INTEGRITY_Z=2.8` validity only. No trade/leg-bps series → PSR N/A. Thin rows retained. |
| §8 golden trace T1/T2/T3 | `analysis.py:90-172,243-244`; `test_exp103_adapter.py:26-34` | MATCHES | Hand-replay of design counts matches T1/T2 (see golden-trace table). T3 strict 0.50 boundary enforced and tested. |
| §9 amendment ledger through AMENDMENT-16 | `design.md` §9; checkpoint §2 | MATCHES | 4 looser / 3 tighter / 8 neutral. AMENDMENT-15 and -16 both LOOSER; streak = 2, not ≥3. False-qualifier count zero by construction (no machine qualification). |
| §10 zero-cost | `contract.py:10-25`; `test_contract.py:32-33`; no cost imports in live modules | MATCHES | Canonical text verbatim. No `evaluation_cost_legacy`, `spread_scale_route`, or `bybit_round_trip_cost_bps` on the live path. No cost directive. |
| Report layers: no machine value labels | `adapter.py:analyze` `interpretation`; grep `SUPPORTED`/`WASH`/`CONTRADICTED` | MATCHES | No those fields in analysis code or shared module. |

### Golden-trace diff

| Event | Expected from design (§8) | Implemented logic (hand-derived, not from running the adapter) | Verdict |
|---|---|---|---|
| T1 tight, counts `[29,12,23,23,27,26]`, bin_width=1, ATR=10 | POC=100; VA 100→104, va_count=114, target=ceil(0.70×140)=98; val=100, vah=105, VA_width=5; gap bins 101+102 (12+23=35≥35); gap_span=2, gap_span_va=0.40, tight=true | `replay_profile`: POC `max(count, -index)` → 100; upper-first expansion to 114; lowest-density gap to 35 → bins 101,102; `2 < 0.5×5` → true | MATCHES |
| T2 non-tight, counts `[10,18,13,7,7,30]` | POC=105; lower expansion 105→101, va_count=75, target=60; val=101, vah=106, width=5; gap 103,104,102 (7+7+13=27≥23); gap_span=3, gap_span_va=0.60, tight=false | Same replay: POC=105; lower side (upper sentinel −1); span=3; `3 < 2.5` → false | MATCHES |
| T3 strict boundary | VA_width=4, gap_span=2 → ratio 0.50 → tight=false (`<` not `≤`) | `validate_profile_frame` `VOID_TIGHT_GAP_BOUNDARY` iff `tight_gap != (span < 0.5×width)`; adapter test keeps a 0.50/false row passing | MATCHES |
| Fixture plants on those labels | Tight arm ← T1, non-tight ← T2; raw contrasts +0.50 / +3.6e12 ns / +0.25 | `fixture_frame` maps `tight_gap` True→T1, False→T2; receipt `raw_estimate` 0.5, 3600000000000.0, 0.25; `destroyed_draws` 2000; top-level `blocking_pass` true | MATCHES |

### Governance & boundary

- **Fresh context:** PASS — dedicated subagent; this conversation did not implement the code.
- **Gate-first / 264 TRAIN cells:** PASS — family + per-cell gates before parquet outcome reads; `expected_cells=264`; TRAIN end `2023-11-22T00:00:00Z` = `1_700_611_200` s; start `2021-06-02T00:01:00Z`.
- **Holdout / TEST:** PASS — `endpoint_ts_ns <= train_end` in scan and validation; no TEST path; no holdout query.
- **Registry:** PASS — `CF-LIQSWP-001/HYP-003` → EXP-103 (`cf-liqswp-001.md:121`). No counted TEST read declared.
- **No Python price backtest:** PASS — analysis-only.
- **No local accounting:** PASS by inspection of `analysis_code/` (no adjudication/P&L primitives). Supervisor should still run `check_no_local_accounting("python/experiments/EXP-103/analysis_code")` (not run here).
- **Zero cost:** PASS — disclosure verbatim; no live cost function.
- **No research powering:** PASS — denylist clean; `INTEGRITY_Z=2.8` validity only.
- **Derangement (L-28):** PASS — rejection sampling, zero fixed points.
- **AMENDMENT-15/16:** PASS — destroyed mean vs raw SE; singletons fixed and disclosed; void only no-movable / no-changed-value.
- **One BacktestNode:** PASS — EXP-103 constructs none; source re-checks `one_backtest_node=true`.
- **60m labels:** PASS — observation hours emit `60m`.
- **PSR / screen-pin / XENA / COST-DIRECTIVE:** N/A.
- **Battery/eligibility (L-24):** N/A beyond F07 (every row retained). Design states F02/F04/F06 N/A.
- **Amendment streak:** PASS — final 4L/3T/8N; looser streak 2.
- **Mandatory design blocks:** PASS — mechanism, object-identity, control, tripwire, operator-only bands, sample-size, golden trace, hard/informative split, zero-cost, amendment ledger. No SPDR conversion pin needed.
...
1. Live control JSON keeps destroyed mean, empirical 95% interval, and `destroyed_draws`; it omits the raw 2,000-draw vector (`test_exp103_analysis_live.py:123-140`). The registered bite uses the mean. Design §6 lists “all 2,000 destroyed contrasts” among disclosures.
2. Fence `manifest_sha256` is compared to the pinned constant; the manifest file is opened for start/pin fields but its bytes are not re-hashed.
3. `all_defined_baseline` is the `DEFINED` pool, not re-applied to the completed-primary mask. It is not the comparator.

Run-9 REVISE items independently re-checked and closed: §1 seal fields (`emission_contract_version`, Nautilus `1.230.0`, `one_backtest_node`, manifest SHA, TRAIN start); §5 computable VA/gap thresholds plus design clarification; singleton destroy groups under AMENDMENT-16.

Supervisor commands not run in this read-only pass: `check_no_local_accounting("python/experiments/EXP-103/analysis_code")`; `pytest python/tests/liqswp_analysis/test_exp103_adapter.py python/tests/test_exp103_analysis_live.py python/tests/liqswp_analysis/test_source.py`.
