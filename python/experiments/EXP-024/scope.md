...
- **Parameters**: Compare confirmation-close proxy, immediate next-open, second-candle-open, and first deterministic retest entry after the same approved IFVG or breaker confirmation event; first retest means the first later bar whose high/low touches the confirmation zone before invalidation, using only bars after confirmation; no new filters. All timing variants keep the inherited confirmation-source stop. If any timing rule's inherited stop distance falls below the carried-forward minimum feasible risk from the confirmation source, count the row in timing diagnostics but exclude it from R-based outcome and slippage summaries as infeasible under the inherited-stop convention.

## Success / Failure Criteria

- **Evidence FOR**: second-candle-open has equal or better expectancy/MAE than simpler entries on at least 3 instruments without worse slippage proxy and with >= 50 risk-feasible confirmation-close and second-candle-open comparisons per train/test segment.
...
Requires an approved confirmation event definition from EXP-021 or EXP-023. This isolates execution timing and must not change confirmation filters, stops, targets, or event eligibility. The confirmation source must already carry the inherited-stop feasibility guard fields needed to exclude near-zero-risk timing rows from R-based comparison.
...
Use sorted 1-minute bars by `CloseTime` for each available instrument. Convert timestamps to New York time where scoped. Apply the nested chronological split before any feature, event, or outcome analysis, and never materialize the final 30 percent holdout. Carry forward the confirmation source's minimum feasible inherited risk value for every timing variant.
