"""SPDR-006 — CF-HTFCAP-001 vol-regime facet TRAIN-only availability screen.

Lane: docs/references/spdr-lane.md · design: python/experiments/SPDR-006/design.md
Inherits SPDR-004 mechanics (AMENDMENTS 1–5) with HTF filter axis replaced by vol-regime
levels: VOL_HI, VOL_LO, DI×VOL_HI, DI_ADX×VOL_HI (1440 treatment cells).

Integrity: design §6 code-asserted checklist must print PASS 14/14 before promote-facing
...
BASES = ("UNF", "MOM", "RAND")
# Treatments only — no DI-only / DI_ADX-only (frozen in SPDR-004). NONE is baseline only.
HTF_FILTERS_TREAT = ("VOL_HI", "VOL_LO", "DI×VOL_HI", "DI_ADX×VOL_HI")
INTERACTION_FILTERS = ("DI×VOL_HI", "DI_ADX×VOL_HI")
STANDALONE_VOL_FILTERS = ("VOL_HI", "VOL_LO")
FROZEN_DIR_MAP = {"DI×VOL_HI": "DI", "DI_ADX×VOL_HI": "DI_ADX"}
ADX_PERIOD = 14
ATR_PERIOD = 14
ADX_GATE = 25.0
VOL_HI_THRESH = 1.25
VOL_LO_THRESH = 0.8
VOL_MEDIAN_W = 100  # HTF bars for rolling-median ATR
RAND_SEEDS = list(range(1000, 1025))  # 25 seeds, regenerable (L-19)
PHASE_SHIFT_K = 50  # Control C: K HTF bars joint vol+DI+ADX (design §4)
N_BOOT = 1_000  # screen-practical; dependence-honest with block rule below
N_CI_SEEDS = 3
MIN_TRADES_POWER = 30
# Cell grid top-N by membership-days (design 1440 = 10×3×4×3×4); membership series still full.
PRIMARY_N = 10
STAGING_BARS = REPO / "python/experiments/INFR-011/data/staging/bars"
SPDR004_CELLS = REPO / "python/experiments/SPDR-004/results/cells.parquet"
SPDR004_MEMBERSHIP = REPO / "python/experiments/SPDR-004/results/membership.parquet"
...
class DomainCtx:
    ...
    adx: np.ndarray
    vol_ratio: np.ndarray  # HTF ATR(14)/rolling-median ATR(W=100), mapped last-closed
    member: np.ndarray  # bool: symbol in top-10 at this LTF bar open
    valid_htf: np.ndarray  # HTF DI/ADX features finite + mapped
    valid_vol: np.ndarray  # vol_ratio finite + mapped
    ...
    _vol_ratio_raw: np.ndarray = field(repr=False, default_factory=lambda: np.array([], dtype=np.float64))
    _map_idx: np.ndarray = field(repr=False, default_factory=lambda: np.array([], dtype=np.int64))
    _joint_shift_applied: bool = field(default=False)
...
def causal_rolling_median(x: np.ndarray, window: int) -> np.ndarray:
    ...
    if n == 0 or window < 1:
        return out
    # O(n·W) is fine for HTF lengths (≤ tens of thousands)
...
def build_domain_ctx(
    ...
    *,
    ...
    # HTF ATR(14) at each closed HTF bar; vol_ratio = ATR / rolling-median(W=100)
    atr_htf = wilder_atr(h_high, h_low, h_close, ATR_PERIOD)
    med_atr = causal_rolling_median(atr_htf, VOL_MEDIAN_W)
    with np.errstate(divide="ignore", invalid="ignore"):
        vol_ratio_raw = atr_htf / med_atr
    vol_ratio_raw[~np.isfinite(vol_ratio_raw) | (med_atr <= 0)] = np.nan

    joint_shift = False
    if derange_htf:
        # L-28 Control C alternate path: joint derange of ALL HTF gate inputs
        ...
        adx = adx[perm]
        vol_ratio_raw = vol_ratio_raw[perm]
        joint_shift = True
    elif shift_htf > 0:
        # Control C primary: phase-shift vol + DI + ADX JOINTLY by K HTF bars
        htf_dir = np.roll(htf_dir, shift_htf)
        adx = np.roll(adx, shift_htf)
        vol_ratio_raw = np.roll(vol_ratio_raw, shift_htf)
        # invalidate wrap-around contamination
        htf_dir[:shift_htf] = 0
        adx[:shift_htf] = np.nan
        vol_ratio_raw[:shift_htf] = np.nan
        joint_shift = True
    ...
    vol_ratio_ltf = np.where(valid_map, vol_ratio_raw[mm], np.nan)
    valid_htf = (
        valid_map
        & (htf_dir_ltf != 0)
        & np.isfinite(adx_ltf)
    ...
    return DomainCtx(
        ...
        _adx_raw=adx,
        _vol_ratio_raw=vol_ratio_raw,
        _map_idx=m,
        _joint_shift_applied=joint_shift,
    )
...
def _vol_hi(ctx: DomainCtx) -> np.ndarray:
    return ctx.valid_vol & np.isfinite(ctx.vol_ratio) & (ctx.vol_ratio >= VOL_HI_THRESH)


def _vol_lo(ctx: DomainCtx) -> np.ndarray:
    return ctx.valid_vol & np.isfinite(ctx.vol_ratio) & (ctx.vol_ratio <= VOL_LO_THRESH)
...
def entry_mask_and_sign(
    ...
    *,
    ...
    Vol standalone: direction-less gate. UNF×{VOL_HI,VOL_LO} = long-only (drift-exposed).
    DI×VOL_HI / DI_ADX×VOL_HI: sign from DI; vol gates only (amplifier form).
    """
    n = ctx.n
    idx = np.arange(n)
    room = idx + hold < n
    ...
    if htf_filter == "NONE":
        ...
        if base == "MOM":
            ...
            return cadence, ctx.mom_sign.astype(np.int8)
        if base == "RAND":
            if rand_seed is None:
                raise ValueError("RAND requires seed")
            rs = draw_rand_sign(n, rand_seed)
            return base_ok & (rs != 0), rs
        raise ValueError(base)

    # --- standalone vol (direction-less) ---
    if htf_filter in STANDALONE_VOL_FILTERS:
        vgate = _vol_hi(ctx) if htf_filter == "VOL_HI" else _vol_lo(ctx)
        if base == "UNF":
            # declared drift-exposed long-only (design §3)
            return base_ok & vgate, np.ones(n, dtype=np.int8)
        if base == "MOM":
            cadence = base_ok & (ctx.mom_sign != 0) & vgate
            return cadence, ctx.mom_sign.astype(np.int8)
        if base == "RAND":
            if rand_seed is None:
                raise ValueError("RAND requires seed")
            rs = draw_rand_sign(n, rand_seed)
            return base_ok & vgate & (rs != 0), rs
        raise ValueError(base)

    # --- directional × VOL_HI amplifier ---
    if htf_filter in INTERACTION_FILTERS:
        vgate = _vol_hi(ctx)
        dgate = ctx.valid_htf & (ctx.htf_dir != 0)
        if htf_filter == "DI_ADX×VOL_HI":
            dgate = dgate & np.isfinite(ctx.adx) & (ctx.adx >= ADX_GATE)
        elif htf_filter != "DI×VOL_HI":
            raise ValueError(htf_filter)
        gate = dgate & vgate
        if base == "UNF":
            return base_ok & gate, ctx.htf_dir.astype(np.int8)
        if base == "MOM":
            cadence = base_ok & (ctx.mom_sign != 0)
            elig = cadence & gate & (ctx.mom_sign == ctx.htf_dir)
            return elig, ctx.htf_dir.astype(np.int8)
        if base == "RAND":
            if rand_seed is None:
                raise ValueError("RAND requires seed")
            rs = draw_rand_sign(n, rand_seed)
            elig = base_ok & gate & (rs == ctx.htf_dir)
            return elig, ctx.htf_dir.astype(np.int8)
        raise ValueError(base)

    raise ValueError(f"unknown htf_filter={htf_filter!r} (DI/DI_ADX-only forbidden in SPDR-006)")
...
def measure_unit_pin(contexts: list[DomainCtx]) -> dict:
        ...
        rows.append(
            {
                ...
                "domain": ctx.domain,
                "ltf_min": ctx.ltf_min,
                "disclosure_atr": "LTF ATR(14)[t-1] via xen.zigzag.wilder_atr; HTF vol_ratio=ATR(14)/med_ATR(W=100)",
                "atr_file_line": "xen/zigzag.py:wilder_atr + screen_code/spdr006_screen.py:wilder_adx_di+causal_rolling_median",
    ...
    return {
        ...
        "htf_vol_ratio": "HTF ATR(14)[last closed]/rolling-median ATR(W=100 HTF bars)",
...
def measure_train_spread_bps(symbols: list[str], fence) -> dict[str, float]:
    """Per-symbol TRAIN-median pseudo-quote SpreadBps from INFR-011 staging bars."""
    out: dict[str, float] = {}
    train_end = fence.train_end_utc
    # staging CloseTime is timezone-naive μs; compare as naive UTC
    te = train_end.replace(tzinfo=None) if getattr(train_end, "tzinfo", None) else train_end
...
def golden_trace_g1(ctx: DomainCtx) -> dict:
    """G1: 4h/15m DI×VOL_HI — ATR ratio, DI sign from last closed 4h; hand r_bps H=16."""
    if ctx.domain != "4h/15m":
        return {"ok": False, "reason": "wrong domain"}
    hold = 16  # 1× HTF span on 4h/15m
    for t in range(ctx.n):
        if not (ctx.valid_htf[t] and ctx.valid_vol[t] and ctx.member[t] and t + hold < ctx.n):
            continue
        if not (np.isfinite(ctx.vol_ratio[t]) and ctx.vol_ratio[t] >= VOL_HI_THRESH):
            continue
        if int(ctx.htf_dir[t]) == 0:
            continue
        ...
        return {
            "ok": bool(ok_bound and ok_next and s != 0 and np.isfinite(r) and ctx.vol_ratio[t] >= VOL_HI_THRESH),
            "ltf_bar": int(t),
            "htf_idx": m,
            "htf_close_ns": int(ctx._htf_close_ns[m]),
            "ltf_open_ns": int(ctx.ltf_open_ns[t]),
            "htf_dir": s,
            "vol_ratio": float(ctx.vol_ratio[t]),
            "adx": float(ctx.adx[t]) if np.isfinite(ctx.adx[t]) else float("nan"),
            "r_bps_H16": float(r),
            "forming_bar_not_used": bool(ok_next),
        }
    return {"ok": False, "reason": "no valid G1 DI×VOL_HI bar"}


def golden_trace_g2(ctx: DomainCtx) -> dict:
    """G2: 1h/5m VOL_LO standalone UNF long-only; ratio ≤ 0.8 and sign = +1."""
    if ctx.domain != "1h/5m":
        return {"ok": False, "reason": "wrong domain"}
    hold = 12
    elig, sign = entry_mask_and_sign(ctx, "UNF", "VOL_LO", hold)
    for t in range(ctx.n):
        if not elig[t]:
            continue
        m = int(ctx._map_idx[t])
        ok_bound = m >= 0 and ctx._htf_close_ns[m] < ctx.ltf_open_ns[t]
        ok_vol = np.isfinite(ctx.vol_ratio[t]) and ctx.vol_ratio[t] <= VOL_LO_THRESH
        ok_sign = int(sign[t]) == 1
        return {
            "ok": bool(ok_bound and ok_vol and ok_sign and t + hold < ctx.n),
            "ltf_bar": int(t),
            "vol_ratio": float(ctx.vol_ratio[t]),
            "sign": int(sign[t]),
            "htf_close_ns": int(ctx._htf_close_ns[m]) if m >= 0 else -1,
            "exit_open_ns": int(ctx.ltf_open_ns[t + hold]) if t + hold < ctx.n else -1,
            "long_only_convention": True,
        }
    return {"ok": False, "reason": "no VOL_LO UNF G2 bar"}
    ...
    *,
    ...
    items = [
        ...
        },
        {
            ...
            "pass": unit_pin_ok,
            "detail": "measured TRAIN ATR bps + SpreadBps cost pins written",
        },
        {
            "id": 9,
        ...
        {
            "id": 12,
            "name": "Joint phase-shift covers vol+DI+ADX",
            "pass": joint_shift_ok,
            "detail": "Control C shifts every HTF gate input together",
        },
        {
            "id": 13,
            "name": "No DI-only/DI_ADX-only treatment cell",
            "pass": no_dir_only_ok,
            "detail": "frozen-facet guard; treatments ⊆ VOL_HI/VOL_LO/DI×VOL_HI/DI_ADX×VOL_HI",
        },
        {
            "id": 14,
            "name": "Amplifier disclosure table",
            "pass": amplifier_ok,
            "detail": "DI×VOL_HI / DI_ADX×VOL_HI vs frozen SPDR-004 cells emitted",
        },
    ]
    return items
...
def run() -> None:
    ...
    print("[SPDR-006] start", datetime.now(timezone.utc).isoformat())

    reg_ok, reg_msg = check_registration()
    print(f"[SPDR-006] registration: {reg_ok} ({reg_msg})")

    fence = load_fence_manifest()
    catalog = ParquetDataCatalog(str(CATALOG_PATH.resolve()))
    pool = load_admitted_pool()
    print(f"[SPDR-006] admitted listed pool: {len(pool)} symbols")
    ...
    print(f"[SPDR-006] membership rows: {membership.height}")

    # symbols that ever appear in top-10
    member_syms = membership["symbol"].unique().sort().to_list()
    print(f"[SPDR-006] unique member symbols: {len(member_syms)}")
    ...
    print(f"[SPDR-006] primary {PRIMARY_N} by membership-days: {primary_syms}")
    ...
    for sym in tqdm(primary_syms, desc="domain_ctx"):
        ...
        for name, htf_min, ltf_min, ratio in DOMAIN_PAIRS:
            ...
            if name == "4h/15m" and not g1.get("ok"):
                g1 = golden_trace_g1(ctx)
            if name == "1h/5m" and not g2.get("ok"):
                g2 = golden_trace_g2(ctx)

    g3 = golden_trace_g3(membership, daily_by_sym)
    print(f"[SPDR-006] golden G1={g1.get('ok')} G2={g2.get('ok')} G3={g3.get('ok')}")

    # L-21 unit pin + measured per-symbol TRAIN-median SpreadBps (NOT GAP=2)
    unit_pin = measure_unit_pin(contexts)
    primary_for_spread = sorted({c.symbol for c in contexts})
    spread_med = measure_train_spread_bps(primary_for_spread, fence)
    unit_pin["train_median_spread_bps"] = spread_med
    unit_pin["spread_source"] = str(STAGING_BARS)
    unit_pin["spread_note"] = (
        "INFR-011 staging SpreadBps TRAIN median; RT component = full quote width; "
        "fees = BYBIT_USDT_PERP_FEES taker 5.5 bps/side (11 bps RT); funding disclosed GAP=1 bps/8h"
    )
    cost_examples = []
    for sym in primary_for_spread:
        sp = spread_med.get(sym, float("nan"))
                ...
                cost_examples.append(
                    {
                        ...
                        "hold_bars": h_bars,
                        "hold_hours": hold_hours,
                        "spread_bps_measured": float(sp),
                        "cost_proxy": c,
                        "note": "measured TRAIN-median SpreadBps (not GAP=2)",
                    }
                )
    ...
    print(f"[SPDR-006] spread pins: { {k: round(v, 4) if np.isfinite(v) else None for k,v in spread_med.items()} }")

    # L-28 derangement self-check + joint-shift coverage (item 12)
    derange_ok = True
    joint_shift_ok = True
    ...
    if contexts:
        ...
        derange_ok = derange_ok and (c_d is not None) and bool(getattr(c_d, "_joint_shift_applied", False))
        # phase-shift ctx must flag joint shift and have vol_ratio field shifted
        if contexts_shift:
            cs0 = contexts_shift[0]
            joint_shift_ok = bool(cs0._joint_shift_applied) and cs0.vol_ratio is not None
            # sanity: shifted vol series differs on some valid bars when unshifted has data
            if c0.symbol == cs0.symbol and c0.domain == cs0.domain:
                both = np.isfinite(c0.vol_ratio) & np.isfinite(cs0.vol_ratio)
                if np.any(both):
                    # not all equal after nontrivial shift (allow rare coincidence)
                    joint_shift_ok = joint_shift_ok and (
                        not np.allclose(c0.vol_ratio[both], cs0.vol_ratio[both], equal_nan=True)
                        or int(both.sum()) < 10
                    )
        else:
            joint_shift_ok = False
    else:
        joint_shift_ok = False
    ...
    print(f"[SPDR-006] cells: {cells.height} (treatment={n_treat}, baseline={n_base})")
    ...
    unit_pin_ok = (
        ...
        and len(unit_pin.get("train_median_spread_bps", {})) > 0
        and all(np.isfinite(v) for v in unit_pin.get("train_median_spread_bps", {}).values())
    )
    ...
    # Item 13: no DI-only / DI_ADX-only treatment cells
    treat_filters = set(
        cells.filter(pl.col("is_treatment"))["htf_filter"].unique().to_list()
    ) if cells.height else set()
    banned_dir = {"DI", "DI_ADX"}
    no_dir_only_ok = treat_filters.isdisjoint(banned_dir) and treat_filters.issubset(
        set(HTF_FILTERS_TREAT)
    )

    # Item 14: amplifier disclosure vs frozen SPDR-004 (read-only)
    amp_rows: list[dict] = []
    amplifier_ok = False
    if SPDR004_CELLS.exists():
        fr = pl.read_parquet(SPDR004_CELLS)
        fr_t = fr.filter(pl.col("is_treatment"))
        inter = cells.filter(
            pl.col("is_treatment") & pl.col("htf_filter").is_in(list(INTERACTION_FILTERS))
        )
        for row in inter.iter_rows(named=True):
            frozen_filt = FROZEN_DIR_MAP[row["htf_filter"]]
            hit = fr_t.filter(
                (pl.col("symbol") == row["symbol"])
                & (pl.col("domain") == row["domain"])
                & (pl.col("hold_mult") == row["hold_mult"])
                & (pl.col("base") == row["base"])
                & (pl.col("htf_filter") == frozen_filt)
            )
            if hit.height == 0:
                continue
            f0 = hit.row(0, named=True)
            this_mean = row["mean_bps"]
            fr_mean = f0["mean_bps"]
            this_lift = row["lift_bps"]
            fr_lift = f0["lift_bps"]
            amp_rows.append(
                {
                    "symbol": row["symbol"],
                    "domain": row["domain"],
                    "hold_mult": row["hold_mult"],
                    "hold_bars": row["hold_bars"],
                    "base": row["base"],
                    "htf_filter": row["htf_filter"],
                    "frozen_filter": frozen_filt,
                    "mean_bps": this_mean,
                    "lift_bps": this_lift,
                    "lift_ci_low": row["lift_ci_low"],
                    "lift_ci_high": row["lift_ci_high"],
                    "lift_ci_excludes_zero": row["lift_ci_excludes_zero"],
                    "n_trades": row["n_trades"],
                    "unpowered": row["unpowered"],
                    "destroy_collapse_frac": row["destroy_collapse_frac"],
                    "frozen_mean_bps": fr_mean,
                    "frozen_lift_bps": fr_lift,
                    "frozen_lift_ci_low": f0.get("lift_ci_low"),
                    "frozen_lift_ci_excludes_zero": f0.get("lift_ci_excludes_zero"),
                    "frozen_n_trades": f0.get("n_trades"),
                    "amp_mean_minus_frozen_mean": (
                        float(this_mean - fr_mean)
                        if np.isfinite(this_mean) and np.isfinite(fr_mean)
                        else float("nan")
                    ),
                    "amp_lift_minus_frozen_lift": (
                        float(this_lift - fr_lift)
                        if np.isfinite(this_lift) and np.isfinite(fr_lift)
                        else float("nan")
                    ),
                }
            )
        amplifier_ok = len(amp_rows) > 0 and len(amp_rows) == inter.height
    else:
        amplifier_ok = False

    # membership sha vs SPDR-004 if present
    import hashlib as _hashlib

    mem_sha = _hashlib.sha256((RESULTS / "membership.parquet").read_bytes()).hexdigest()
    spdr004_mem_sha = None
    mem_byte_identical = False
    if SPDR004_MEMBERSHIP.exists():
        spdr004_mem_sha = _hashlib.sha256(SPDR004_MEMBERSHIP.read_bytes()).hexdigest()
        mem_byte_identical = mem_sha == spdr004_mem_sha
    ...
    items = run_integrity(
        ...
        joint_shift_ok=joint_shift_ok,
        no_dir_only_ok=no_dir_only_ok,
        amplifier_ok=amplifier_ok,
    )
    ...
    integrity_payload = {
        ...
        "htf_filters_treat": list(HTF_FILTERS_TREAT),
        "membership_sha256": mem_sha,
        "spdr004_membership_sha256": spdr004_mem_sha,
        "membership_byte_identical_to_spdr004": mem_byte_identical,
        "fence": {
    ...
    if not all_pass:
        print("[SPDR-006] INTEGRITY FAIL — refusing promote-facing cell write")
        sys.exit(2)

    # Promote-facing writes only after PASS 14/14
    cells.write_parquet(RESULTS / "cells.parquet")
    cells.write_csv(RESULTS / "cells.csv")
    amp_df = pl.DataFrame(amp_rows, strict=False) if amp_rows else pl.DataFrame()
    if amp_df.height:
        amp_df.write_parquet(RESULTS / "amplifier_vs_spdr004.parquet")
        amp_df.write_csv(RESULTS / "amplifier_vs_spdr004.csv")
    ...
    summary = {
        ...
        "htf_filters": list(HTF_FILTERS_TREAT),
        "vol_hi_thresh": VOL_HI_THRESH,
        "vol_lo_thresh": VOL_LO_THRESH,
        "vol_median_w": VOL_MEDIAN_W,
        ...
        "money_floor": "TRAIN-median staging SpreadBps + taker 5.5 bps/side + funding GAP disclosure",
        "train_median_spread_bps": spread_med,
        "n_cells_total": cells.height,
        "n_treatment": n_treat,
        "n_baseline": n_base,
        "n_primary_treatment": prim.height,
        "n_amplifier_rows": len(amp_rows),
        "primary_symbols": primary_syms,
        ...
        "membership_sha256": mem_sha,
        "membership_byte_identical_to_spdr004": mem_byte_identical,
        "note": "Pooled figures disclosure-only. No disposition in this artifact. K=3 on THIS grid only.",
    ...
    print(f"[SPDR-006] wrote results → {RESULTS}")
