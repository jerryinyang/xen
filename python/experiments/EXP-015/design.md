...
· **Phase:** post-004 (retro pending) · **Type:** mechanism characterisation, TRAIN.
...
| **M3 — attribution (drift / vol / reversion)** | Is the Part-A P&L dislocation-conditional or exposure? | (a) Per cell: Part-A extend-arm net vs the **same cells' shift twins** per ladder level — **collapse fraction (shift net / raw net) disclosed per level** (W3; binary reads prohibited as the only statement). (b) Direction split: ΔR_24 for with-drift vs against-drift events (sign of trailing 90-bar return); a "reversion" that exists only against the prevailing 5y drift direction is exposure, not MR. (c) Event-time overlap census: % of Part-A ladder P&L accrued while ≥2 legs open (scale-in dependency). | Uses only existing emissions + Part-B measurements; separates the three candidate explanations without new machinery. |
...
L-13 (ladder-native availability, not the two-barrier race), W3 (collapse fractions disclosed),
