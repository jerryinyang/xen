...
def certify_and_rank(finalists: list[RestartResult], streams: list[CandidateStream],
    ...
    and are recorded as retired. INFR-022: gross-only (no NET companion — zero-cost
    model). No verdict: operator judges.
    ...
    # NET companion: RETIRED under the INFR-022 zero-cost model — no cost enters any
    # calculation programme-wide, so there is no net read to companion the gross package.
    g_net_top = {
        "binding": False,
        "retired": True,
        "note": "NET companion retired (INFR-022 zero-cost model): gross-only evidence "
                 "package; deployability/tradability claims remain refused by rule",
    }
    ...
    return {
        ...
        "net_companion": g_net_top,            # RETIRED under INFR-022 (gross-only)
        "ranked": ranked,
        ...
        "retired_binders": [
            ...
            "S_as_pass_threshold", "net_companion",
        ],
        "binding_note": (
