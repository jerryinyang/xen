...
Produces 15-minute (or any N-minute) bars from 1-minute base bars.
...
def aggregate_ohlc(
    ...
    - ``min_coverage is None`` (default): a window is retained only when it
      contains exactly ``period_minutes`` 1-minute bars. This is the default,
      strict behavior for callers that do not pass the argument.
    ...
    bars_1m : pl.DataFrame
        ...
        and is summed when present. Must be sorted by `CloseTime`
        (non-decreasing); this function does not reorder input and raises
        `ValueError` on unsorted input.
    ...
    if not bars_1m.get_column("CloseTime").is_sorted():
        raise ValueError(
            "bars_1m must be sorted by CloseTime (non-decreasing) before aggregation; "
            "this function does not reorder input."
        )
