#!/usr/bin/env bash
# INFR-002 — New-universe 1-minute time-bar collection (Phase 010 Track C).
#
# Runs the Xen robot in TimeBars mode (Mode=1, CollectTimeBars=true) over the
# 13 new-universe instruments declared in the Phase 010 design §5/C1:
#   GBPUSD USDJPY USDCHF USDCAD AUDUSD NZDUSD EURJPY GBPJPY AUDJPY
#   US500 US2000 DE30 JP225
# (USTEC is already local and is NOT re-collected.)
#
# HOLDOUT POLICY (binding from the moment each file lands): the final 30% of
# each new instrument's chronologically ordered dataset is GLOBAL HOLDOUT,
# sealed under the standard rules; the first 70% splits 70/30 TRAIN/TEST on
# the 1-minute-row timestamp convention. NO experiment reads any new-universe
# row before VAL-003 (python/experiments/VAL-003/) passes.
#
...
# INFR002_MAX_PARALLEL).
#
# Usage:
#   ./run-infr002-collection.sh              # all symbols, pool of 4
#   ./run-infr002-collection.sh one GBPUSD   # single symbol
#   ./run-infr002-collection.sh metadata     # robot parameter listing
#   INFR002_MAX_PARALLEL=2 ./run-infr002-collection.sh
#
...
#
# Broker symbol names for the index CFDs vary (e.g. DE30 vs GER40, US500 vs
# SPX500). If a symbol is rejected, find the broker's name with the cTrader
# app and run:  ./run-infr002-collection.sh one <BROKER_SYMBOL>
#
...
MAX_PARALLEL="${INFR002_MAX_PARALLEL:-4}"
MAX_WAIT="${CTRADER_RUN_MAX_WAIT_SECONDS:-14400}"
...
# Collection window. Start matches the existing universe (02/01/2023); the CLI
# begins wherever the broker's m1 history actually starts if later. The end
# date defines where the sealed final-30% holdout boundary will fall — record
# any override in the INFR-002 registry row.
COLLECT_START="${INFR002_START:-02/01/2023 00:00}"
COLLECT_END="${INFR002_END:-11/06/2026 00:00}"
...
# New-universe instrument set (design §5/C1). Override with INFR002_SYMBOLS
# (space-separated) for broker-specific names.
if [[ -n "${INFR002_SYMBOLS:-}" ]]; then
  read -r -a symbols <<<"$INFR002_SYMBOLS"
else
  symbols=(GBPUSD USDJPY USDCHF USDCAD AUDUSD NZDUSD EURJPY GBPJPY AUDJPY
           US500 US2000 DE30 JP225)
fi
...
run_collection() {
  ...
  local container="ctrader-infr002-$(printf '%s' "$symbol" | tr '[:upper:]' '[:lower:]')-$$"
  local before_files
  local cid
  local log_pid=""
  local start
...
if [[ "${1:-}" == "one" ]]; then
  ...
  symbols=("$2")
  if [[ -z "${INFR002_SKIP_CACHE_PREP:-}" ]]; then
    prepare_cache_layout
  fi
  run_collection "$2"
  exit $?
...
status=0
printf '%s\n' "${symbols[@]}" \
  | INFR002_SKIP_CACHE_PREP=1 xargs -n1 -P "$MAX_PARALLEL" "$0" one \
  || status=1

if [[ "$status" -eq 0 ]]; then
  echo "Done. New-universe time bars are under $ROOT_DIR/data/timebars."
  echo "Next: run VAL-003 (python/experiments/VAL-003/) before any experiment touches this data."
else
