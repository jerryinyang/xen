"""Generate the XENA-001 universe manifest (design.md §3 grid, 2,736 candidates).

Writes ``data/strategy_runs/XENA-001/universe_manifest.json`` in the
...
MtfCtxRandomModel emits — no timestamp suffix.
...
money_per_unit: design §4 pins (TRAIN-window FX medians / HKD peg; QA run 1
amendment 4).

Usage:  python tools/ctrader-cli/experiments/gen_xena001_manifest.py
"""
...
OUT_DIR = ROOT / "data" / "strategy_runs" / "XENA-001"
...
def main() -> None:
        ...
        for pair in DOMAIN_PAIRS:
            for hold in HOLD_LABELS:
                for v in range(N_VARIANTS):
                    cid = f"C1-{symbol}-{pair}-{hold}-V{v:02d}"
                    candidates.append({
                        "candidate_id": cid,
                        "run_dir": cid.lower(),
                        "symbol": symbol,
    ...
    manifest = {
        ...
        "model": "MtfCtxRandom (CTRL-01 RANDOM, lambda=2)",
