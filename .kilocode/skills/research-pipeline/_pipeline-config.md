...
Each robot session produces **seven compressed Parquet files** — the cleansed tick archive, TriLattice feature stream, per-stream bar logs, a pivot log, and a traditional time-bar baseline. See `docs/references/dataset-reference.md` for authoritative column definitions, schema details, and loading patterns.

When `NoiseFlipRate > 0`, the same seven files are produced under `data/noise/noise_XXX/` (where XXX = flip rate × 100, zero-padded to 3 digits, e.g. `noise_010`, `noise_020`, `noise_030`). The tick archive always writes to `data/ticks/` regardless of noise mode. All other schemas are identical to their baseline counterparts. See `docs/references/dataset-reference.md` for noise injection architecture, cTrader parameters, and noise-specific loading patterns.

### 0. Cleansed Tick Archive (4 columns)

Raw cleansed tick stream from `TickCleanser` (pre-noise-injection). Always writes to `data/ticks/`.

- **Path pattern**: `data/ticks/ticks_<symbol>_<yyyyMMdd_HHmmss>.parquet`
- **Columns**: 4 (`Timestamp`, `Mid`, `Direction`, `EffectiveEpsilon`)
- **Python access**: `pl.read_parquet(path)` or `pl.scan_parquet(path)`

**Key columns**: `Timestamp`, `Mid`, `Direction`, `EffectiveEpsilon`. For the full schema, see `docs/references/dataset-reference.md`.
