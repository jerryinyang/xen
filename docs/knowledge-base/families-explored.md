...
| **Nautilus/Bybit infrastructure (INFR-010..020, VAL-008)** | Engine migration, fenced catalog, emission/reconciliation, signed bars, Bybit calibration and report-layer correction | **COMPLETE as apparatus; cost path corrected in the Chapter-05 preflight.** VAL-008 passed 39/39. Raw aggressor split reconciled 20/20 symbol-days. Stored `SpreadBps` is not quote spread and is pinned `UNUSABLE`; the replacement cost-floor pins are conservative upper bounds, not quotes, validated on only 20 symbol-days. |
