...
**Status:** **OPEN — G0 PASS (2026-06-22).** Phase 019 D0 ratified and frozen; the **D2 admission-gate
...
{0.025,0.05,0.10}, and self-calibrating under inflated per-cell FP. **Pipeline opens at EXP-086 (Screen M).**
...
| --- | --- | --- | --- | --- |
| **EXP-086** | M — `CF-VOLEXP-001/HYP-001` | **Screen M** (TRAIN-only): single-series magnitude availability — SPLIT typical-range + tail/bimodality vs matched random, on compression primitives (HA-harami inside-bar + NR/inside-bar), incl. the two-sided magnitude-budget; admit/exonerate vs the D2b multiplicity-adjusted gate. | 0 / 0 | **AUTHORIZED (G0 PASS, bite GREEN) — scope next (Stage 1).** (Run first: cheapest way to *close the single-series quadrant*; low, tail-concentrated prior.) |
| **EXP-087** | X — `CF-XSECT-001/HYP-001` | **Screen X** (TRAIN-only): cross-sectional relative-strength favourable availability (basket-relative momentum/divergence rank over 16 instruments) vs matched random; admit/exonerate vs the D2b gate. | 0 / 0 | **PENDING — scoped after EXP-086.** (A-priori mechanism favourite; multiplicity gate matters most here — most cells.) |
| **EXP-088** | F — `CF-FLOW-001/HYP-001` | **Screen F** (TRAIN-only, reserved-conditional): order-flow / liquidity-imbalance availability vs matched random; admit/exonerate vs the D2b gate. | 0 / 0 | **RESERVED-CONDITIONAL — opened only if the operator wants a third comparison after M and X.** |
| **G-019** | — | Terminal gate: emit the ranked admit/exonerate/inconclusive inventory; route per the D5 mechanical rule. | 0 / 0 | **PENDING — adjudicated after the slate** (`G-019-gate-criteria.md` → `G-019-gate-review.md`). |
