...
| **EXP-096** | `CF-MR-001/HYP-003` | Noise infusion — realistic 1-minute entry fill (v1/v2-binding/v3); re-derive the portfolio under v2; decide the G-022a holdout-frozen deployable set. | 0 / 0 | PLANNED. |
| **G-022a** | — | Pre-holdout freeze: non-empty deployable set + analysis-set portfolio edge + calibrated/bite-checked statistic + predeclared band ⇒ freeze `G-022a-gate-criteria.md` + `G-022-gate-criteria.md`; else HALT (holdout preserved). | 0 / 0 | PLANNED. |
| **EXP-097** | `CF-MR-001/HYP-003` | Global-holdout release — one-shot OOS-final confirmation of the frozen noise-aware portfolio (binding) on the final-30% global holdout. | **1 global-holdout shot / 0 slots** | PLANNED (gated behind G-022a). |
| **G-022** | — | Terminal gate: DEPLOYABLE_CONFIRMED / DECAYED / INCONCLUSIVE, per the G-022a band. | 0 / — | PLANNED. |
