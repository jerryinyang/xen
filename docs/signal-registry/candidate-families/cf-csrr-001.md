...
**Status:** `REGISTERED (2026-07-06, operator-directed). G0 PENDING` — definitions fixed;
checkpoint-009 scoped; 0 slots consumed, 0 counted TEST reads, holdout sealed.
...
- **Validatory (TRAIN-only, 0 reads):** EXP-023 — tradability of the selected model under honest
  cost + the three-twin battery + TRAIN out-of-sample robustness.
